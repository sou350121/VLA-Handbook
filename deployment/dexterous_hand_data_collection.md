...
## 2.1 数据金字塔（为什么“遥操少量高质”仍绕不过去）

灵巧手的数据形态非常典型地呈现“金字塔”结构（上层少量高质、下层海量但缺信号）：

- **尖端：遥操作/真机示教**  
  - 覆盖“接触窗口”高信息密度片段（对准、卡入、施力、对抗、恢复）
  - 代价高，但往往是突破成功率的最有效数据
- **中层：仿真/RL/半自动脚本（HITL）**
  - 可扩规模，但 sim2real 在接触动力学（摩擦、形变、滑移）上仍是硬坎
- **底层：人类视频**
  - 覆盖物体与场景多样性，但缺少力与精确动作映射

对应的任务拆解与注意事项见：[`../theory/frontier/dexterous_hands_open_can_cards_data_pyramid.md`](../theory/frontier/dexterous_hands_open_can_cards_data_pyramid.md)
...
### 3.4 额外建议：显式采集“失败恢复段”（Recovery Clips）

对开可乐、发牌、插拔、旋拧等接触密集任务，**“失败→自救→继续”** 往往比“顺滑成功一次”更能提升部署可靠性。建议在数据结构里显式保留：

- `failure_mode`（滑移/卡死/对准失败/遮挡导致误判…）
- `recovery_clip`（失败发生前后的一段窗口）
- `recoverability`（是否处于可自救状态）
