...
| VLA 落地为什么失败？ | 泛化差+长任务弱+不可解释，团队已放弃纯VLA路线 | [§12.10](#1210-踩坑吐槽与落地反思真机实战负面经验) |
| 开源数据微调效果极差？ | 开源数据与本机有 gap（视角/零点），自采 100 条 > 开源 1200 条 | [§12.10](#1210-踩坑吐槽与落地反思真机实战负面经验) |
| VLA 中 Language 有用吗？ | 工业界实测 L 不起作用甚至有害，VA/VAM 路线崛起 | [§12.10](#1210-踩坑吐槽与落地反思真机实战负面经验) |
| 加数据为什么不提升？ | action ambiguity + 时间维度 → 监督信号打架，需更多数据 | [§12.10](#1210-踩坑吐槽与落地反思真机实战负面经验) |
...
> **目录**：[13.1 经典黑话](#131-经典黑话帖1-200-沉淀) → [13.2 2026-04 新增](#132-2026-04-新增黑话帖201-232) → [13.3 金句辑录](#133-金句辑录-one-liners)

### 13.1 经典黑话（帖1-200 沉淀）

| 黑话 | 含义 | 出处 |
|------|------|------|
| **VLA 帕金森** | action 输出不平滑，机械臂全程抖动如帕金森患者 | 帖167、帖172（259赞），已成圈内通用术语 |
| **具身提示工程** | 为了让 VLA 模型"识别"物体，精心布置实验场景（如摆货架），本质是人适应模型而非模型适应环境 | 帖164 评论区（科研游击队 vs 康斯坦丁） |
| **把人当 GPU 用** | 大规模采数据需要大量人力重复遥操作——"画个饼让本科生天天搗鼓机械臂采数据" | 帖174（神鹰黑手哥，287赞，66赞评论） |
| **重新发明低通滤波** | VLA 论文解决抖动的方法 = 传统控制早就有的低通滤波/自适应控制，但包装成 contribution | 帖172 评论区（momo，13赞） |
| **L 不管用 / 不学 L 了** | VLA 中 Language 模块实测对 action 无影响——"中间我随便设的结果都是一样的🤡" | 帖164（Youth-chan，15赞）、帖169 多人实锤 |
| **变树懒** | 大模型控制频率太低（3-10Hz），中间不插值的话机器人像树懒一样慢动作 | 帖163 评论区（Wyatt.） |
| **全局最优损失** | 讽刺 VLA 研究者走火入魔到临终遗言都用优化术语——"终于达到了全局最优损失" | 帖175（160赞讽刺文学） |
| **监督信号打架** | 同一视觉观测对应多种正确 action（ambiguity），加数据反而效果变差 | 帖168（AGI Robot，183赞） |
| **外包采数据的🤡** | 国内具身公司大规模外包数据采集，"很多国内具身智能公司都是外包采数据的🤡" | 帖174 评论区（少年遗梦，36赞） |
| **π0.5 纯纯过拟合** | 小数据微调 π0.5 几乎必过拟合——能抓的位置极其有限，毫无泛化 | 帖176（Rose，97赞） |
| **VLA = VA + 营销** | 小鹏 VLA 2.0 实际没有 Language 模块，但营销投入太大继续叫 VLA——"小鹏的vla2.0没有L" | 帖171 + 帖169 评论区（.E，10赞） |
| **纬度是超参数** | 冷启动温度影响电机精度→南方不出问题→"纬度影响 VLA 成功率" | 帖162 评论区（Hugo） |
| **食堂打饭分析 ViT-L/14** | VLA 研究者职业病：看到食堂窗口都想分析视觉特征和取餐动作的时序建模 | 帖175 讽刺文学 |
| **小作坊玩不下去了** | 具身科研门槛（臂+GPU+人力）超出单课题组承受——"都是大军团作战" | 帖174（287赞）评论区（少年遗梦，36赞） |
| **珍爱生命远离 Piper** | 松灵 Piper 臂的代名词——逆解差、无自锁、肌无力、砸地上 | 帖178（bingcm，102赞） |
| **原来 lerobot = 不幸** | LeRobot 框架 bug 多、PR 无人 review、核心维护者疑似离开 | 帖179（134赞）评论区 |
| **VLA 害了我女儿** | 因选 VLA 方向导致延毕的学术惨剧，引发广泛共鸣 | 帖184（延毕的佩吉，451赞） |
| **论文 288Hz 实测 14Hz** | π0 论文报告推理速度 vs 实际复现差 20 倍——论文数字不可信的代名词 | 帖188（哈哈哈，24赞） |
| **95% 一年内爆雷** | 对具身智能初创公司存活率的悲观预测，已成社区口头禅 | 帖186（假猫熊，263赞） |
| **VLA 八股文** | VLA 面试题标准化成"八股"——action tokenization、token 对齐、chunk 原理成必考 | 帖196（布朗先生，181赞） |
| **云端大脑 = 人去楼空** | 达闼机器人崩盘后，"云端架构"成了 PPT 创业的代名词 | 帖195（375赞） |
| **开辟新坑继续卷** | "具身看起来像没得卷的 learning 派跑来开辟了一个新的坑继续卷论文" | 帖174 评论区（炼金博士，41赞） |
| **左右脑互博** | "做 RL 出身看到 VLA 火起来，一边满脑子问号一边顺着老板做，天天左右脑互博" | 帖椰卷评论区（7赞） |
| **VLM 三成功力都没用到** | "我导师说现在 vla 能用到 30% 的 vlm 的能力就不错了"——回复："没那么多🤥" | 帖169 评论区（AAA冰棍批发潇哥+Volcano） |
| **不如做智能家电** | 与其做人形机器人不如做专用机器——"单臂双臂人形胳膊还不能是生活的一部分吧🤡" | 帖椰卷（希望人类别做具身了，59赞） |
| **不搞 VLA 大家还咋玩儿🤡** | "不搞点vla这种新概念出来，大家还咋玩儿"——讽刺 VLA 是圈子自嗨续命工具 | 帖174 评论区（咸味树莓，15赞） |
| **退化为单模态孤立状态** | 讽刺文学："我的感知正在从多模态融合状态退化为单模态孤立状态……快记录" | 帖175 讽刺文学（VLA 研究者临终场景） |
| **小农经济学术界** | "学术界大部分都是原始社会小作坊一般，过着小农经济相互利益交换维持生计的生活" | 帖174 评论区（JK触发器） |

### 13.2 2026-04 新增黑话（帖201-232）⭐

> 双轮采集（4-17 + 4-28）从 32 篇新帖里浓缩出来的新黑话。**特点**：相比帖1-200 的吐槽偏个体感受，本批次更多是**结构性批判**（架构层、ablation 层、生态层）和**圈内信号情报**（LeRobot 内部消息、PI 拒绝 ablation 等）。

#### 模型 / 架构吐槽

| 黑话 | 含义 | 出处 |
|------|------|------|
| **长瘤子的 VLM** | 形容 VLA 架构丑陋——"VLM 上长出来的"vision tower 27 层不知道在自注意力什么东西，最后一层突然和 text embedding 硬拼接 = 一块瘤子 | 帖207（Ivory Seagull，226 赞，源码层面解构） |
| **前面三十多层真的很必要吗** | Vision tower 冗余质疑——既然只用 last hidden state，前 30 多层是 chair？ | 帖207 评论区延伸 |
| **缝合怪 + ACT 改名异步推理** | SmolVLA 把 ACT 的 action chunk 换名叫"异步推理"然后说是自己的——"妥妥缝合怪" | 帖205（尤里卡，详细吐槽 + 评论印证"act 水平怎么对标 pi0"） |
| **loss 0.002 真机寄** | SmolVLA 训练 loss 降到 0.002，但真机 4 类失败模式：发散 / 死循环 / gripper 滞后 / 抓放低成功率 | 帖203（Scrat，笔记本训练完整复现报告） |
| **loss 值无参考性** | pi0.5 训练 30K 步 loss 比 25K 大，但作者用 25K 推理——"loss 值没很大参考性[扶额R]" | 帖204（Lisette_）+ 帖203 共振 |
| **smolvla 也是很抖** | SmolVLA 抖动是系统现象（多人独立印证）——不是某个超参的事 | 帖204 评论凉了个凉（中国澳门，独立印证） |
| **L 不响应已被 ICML 论文化** | "PI0 没有保持 VLM 的语言理解能力，换不同语言指令对抓取结果基本无影响"——ICML2025 VIP 论文 + AGI Robot 100 条数据实测 双重锤实 | 帖202（AGI Robot，172 赞）+ §14.1 三方印证 |
| **vision-guide 才是真理** | 不用 language guide，用 vision guide。instruction 没有 ground truth 但 vision 有 | 帖201（李兴航 VISTA，2h 数据 → OOD 21 物体 69% vs pi0 14%） |

#### 真机 RL & DAgger 真相

| 黑话 | 含义 | 出处 |
|------|------|------|
| **DAgger 拯救了世界** | DAgger 让真机 RL 样本效率 ×100，且隐含人类复杂偏好——但它"不够 RL"（无 reward model / value function / advantage） | 帖212（ronfjkks 245 赞，必读论述） |
| **PI 0.6\* 都选择不回答这个问题** | "没有 DAgger 你的算法还 work 吗？强如 PI 0.6\* 都选择不回答这个问题"——指 PI 系列拒绝做"无 DAgger ablation"——residual RL + DAgger 才是 PI 的真实底色 | 帖212 |
| **组内物料都被干烂了** | 真机 RL 物料消耗速度超过研究进展——"组内各种物料被干烂了好多" | 帖1（Jupiter Zhai PI 组内，3583 赞，2026-04-17 批次） |
| **黑色孔抠图对准就好了** | Winlone 评论犀利：当前 RL 任务可能被传统视觉方法直接吊打——"这种东西为什么要训练，黑色孔抠图对准就好了" | 帖1 评论（广东，377 赞） |
| **看得到才能称之为智能** | 闭眼用脑子学不到精髓——讽刺纯 RL 不要视觉的极端派 | 帖1 评论（卖宝马的小江江） |
| **填表格 = 真机 demo** | 学术界已经默认 VLA 真机 demo = 填表格——A12 神回 178 赞 | 帖225（曾经的你，244 赞） |

#### Paper factory & review 共识

| 黑话 | 含义 | 出处 |
|------|------|------|
| **填个表格完事了** | "VLA 论文真上过真机吗？打开 GitHub 真机部署的教程也没有" → 评论 A12 神回，**178 赞**——已成评论区高赞共识 | 帖225 评论 A12（上海，178 赞，逆共识升格信号） |
| **完形填空说是** | 同上的 follow-up——VLA 论文 review = 完形填空 | 帖225 评论 AA111AEIOU（广东，23 赞） |
| **现在 vla 还不如小模型** | "vla 还不如小模型 / 小模型没泛化换个位置就废了"——VLA 价值定位被双向夹击 | 帖225 评论 momo + Jeanne |
| **替大家踩了** | 复现失败的牺牲奉献体——"坑我替大家踩了，这篇文章不用细看了" | 帖205（尤里卡） |
| **24 年就开始探索 wam 了** | 银河通用回应"为啥 wam 火一个月就发 RSS 论文"——并不是追热点（搜 dywa） | 帖209 评论（超级燃的具身智能，24 赞） |
| **WAM 强势来袭，但好像还差些啥** | 277 赞质疑：WAM 表面赢了 VLA，但缺关键 piece（评论 Hao Li 答：力反馈/触觉是输入模态问题，不是建模能解决的） | 帖208（peninsula ironbox，527 赞） |

#### 入门门槛 & 工程社区

| 黑话 | 含义 | 出处 |
|------|------|------|
| **想啥呢？没有机械臂玩个锤子** | SO101 教程开篇——"显然不能。想啥呢？没有机械臂，玩个锤子 / 但如果不小心有台机械臂，那就不一样了" | 帖219（具身薯风啸，BOM 3400 元，单 MacBook Pro 训推） |
| **3400 元 + MacBook 起步** | VLA 入门门槛被打到地板——SO101 双臂主从总价 3400 元（电机1200+本体1250+摄像头700+...）配 MacBook Pro 即可闭环训推 ACT | 帖219 |
| **是让 ai 哈，不是让你儿子** | LeRobot 育儿向训练评论——网友建议作者让儿子在虚拟化环境做"成功加分失败扣分"训练后导入机械臂，作者补一句"是让 ai 哈" | 帖221 评论聪明的钢弹（加拿大，38 赞） |
| **Remi 走了以后他们人变少了** | LeRobot 维护降级内部信号——"自从 Remi 走了以后他们人变少了，加上他们最近在搞另外一个大新闻，对已有代码维护就不太上心了" | 帖214 评论 kkkkkk（英国，17 赞，本批最强内部情报） |
| **lerobot most time 会变得不幸** | LeRobot bug 多 / PR 无人 review 的认知确认——"我是直接放弃了 原理 lerobot most time 会变得不幸" | 帖214 评论 maomao（北京，9 赞） |

#### 仿真 / sim2real 工程黑话

| 黑话 | 含义 | 出处 |
|------|------|------|
| **mujoco 角速度 = local 一周陷阱** | mujoco root 角速度 `qvel[3:6]` 是 local，isaac gym 是 global——堵了一周的坑，sim2sim 必查 | 帖217（🐱重庆，pin-point 救命贴） |
| **应力集中神经网络捕捉不到** | 固体力学派对 Real2Sim2Real 的反对——"想做高精度的模拟，同学们多多学习数学力学，做出解析解，而不是依赖数据" | 帖216 评论爱狗小张（江苏，理论派逆共识） |
| **触觉是残差不是主决策** | "触觉不应该承担'决定动作'的角色，更适合用来'修正动作'"——触觉 World Model 的真实价值是**安全 + 失败预警**，不是高保真模拟 | 帖213（R&B All Night，185 赞） |
| **1200 高 > 5688 普通** | HF 叠衣服关键发现——1200 条高质量数据微调使成功率从 40% → 90%，**数据质量远比数量重要** | 帖206（HF 法国官方，194 赞，工程结论） |
...
**关于 VLA 的本质**：
- "**VLA 都是从 VLM 上长出来的**——仔细观察可以发现 Vision 部分一个巨大的 vision tower（27 层）不知道在自注意力什么东西"（Ivory Seagull，帖207）
- "**instruction 没有 ground truth，但 vision 有**。直接告诉模型'去哪里 + 做什么'，不就解决了吗？"（李兴航 VISTA，帖201）
- "**在当前数据规模下不适合用语言和具身智能模型交流**"（AGI Robot ICML2025 VIP 论文，帖202 引用）

**关于 paper 与真机的鸿沟**：
- "**打开 GitHub，真机部署的教程也都没有，所以他们的模型真的上过真机吗？**"（曾经的你，帖225，244 赞引爆 178 赞神回）
- "**填个表格完事了**"（A12 上海，帖225 评论，178 赞）

**关于 RL 的真相**：
- "**强如 PI 0.6\* 都选择不回答这个问题**"——指拒绝做"无 DAgger ablation"（ronfjkks，帖212）
- "**这种东西为什么要训练，黑色孔抠图对准就好了**"（Winlone 广东，帖1 评论，377 赞）

**关于工程的真相**：
- "**真到落地时，真正耗时间的，常常不是模型本身，而是工程适配**"（魔搭 ModelScope，帖215）
- "**瓶颈不是模型，而是数据。高质量 + 一致性数据 = 决定性优势**"（HF 法国官方，帖206）
- "**自从 Remi 走了以后他们人变少了**"（LeRobot 内部消息，帖214 kkkkkk）

**关于触觉与安全**：
- "**一个'足够好'、以安全为导向的触觉 world model，可能比一个高保真但难以部署的物理仿真模型更有实际价值**"（R&B All Night，帖213）

**关于硬件趋势**：
- "**七轴 + 三节升降柱 + 三轮三转底盘 = 2026 中端人形新标配**"（小白学具身 Sunday spec，帖218）
- "**500 个人在家里戴手套采，比 π0.5 的 100 套强很多**"（同上，UMI 数据采集模式产品化）

**关于学术怪圈**：
- "**也许我们可以放下对 PPO/SAC/GRPO 的执念，回到策略提升的框架中重新看待自己的方法**"（ronfjkks，帖212）
- "**作者并不是在追热点，团队 24 年就开始探索 WAM 了，可以搜下 dywa**"（银河通用，帖209，反"蹭热点"自辩）
