...
flowchart LR
  subgraph J["你听到的梗"]
    direction TB
    j1["VLA 帕金森<br/>重新发明低通滤波"]
    j2["不学 L 了<br/>VLA = VA + 营销"]
    j3["pi0.5 纯纯过拟合<br/>监督信号打架"]
    j4["纬度是超参数<br/>远离 Piper"]
    j5["DAgger 拯救了世界<br/>物料都被干烂了"]
    j6["VLA 八股文<br/>论文 288Hz 实测 14Hz"]
    j7["触觉是残差不是主决策"]
    j8["95% 一年内爆雷<br/>云端大脑人去楼空"]
  end
  subgraph R["它指向的真实痛点"]
    direction TB
    r1["action chunk 衔接与平滑性<br/>控制频率不足需插值"]
    r2["语言模态贡献存疑<br/>视觉主导 · 底座是否可去"]
    r3["小数据微调不出泛化<br/>多来源监督互相冲突"]
    r4["硬件一致性与标定<br/>温度/零点/仿真约定"]
    r5["真机 RL 成本极高<br/>本质常退化为 DAgger"]
    r6["指标不可复现<br/>论文数字与真机落差"]
    r7["高频脆弱信号定位错<br/>不该承担主决策"]
    r8["商业化窗口与落地压力"]
  end
  subgraph S["去读这里"]
    direction TB
  ...
  j7 --> r7 --> s7
  j8 --> r8 --> s8
  classDef joke fill:#fff3bf,stroke:#f59f00,color:#000
  classDef pain fill:#ffe3e3,stroke:#f03e3e,color:#000
  classDef sec  fill:#d3f9d8,stroke:#2f9e44,color:#000
  class j1,j2,j3,j4,j5,j6,j7,j8 joke
  class r1,r2,r3,r4,r5,r6,r7,r8 pain
  class s1,s2,s3,s4,s5,s6,s7,s8 sec
```

### 😂 图 6：具身炼丹师的一年（心态曲线 · 全程用黑话标注）

> 纵轴是心情（1 = 想退学，5 = 天下无敌）。**这条曲线本身就是黑话的时间顺序**：先抖动、后祛魅、最后和解。

```mermaid
journey
    title 具身炼丹师的一年（心态曲线 · 附黑话）
    section 入坑期
      读到 pi0 论文 感觉世界的大门打开了: 5: 我
      3400 元加 MacBook 就能起步: 5: 我
      想啥呢 没有机械臂玩个锤子: 2: 我
    section 跑通期
      第一个 ACT 复现成功 激动到流泪: 5: 我
      赛博夹橙子 超级解压: 5: 我
      loss 0.002 以为自己是天才: 4: 我
    section 真机期
      上真机 全程 VLA 帕金森: 1: 我
      loss 0.002 真机寄: 1: 我
      一调一个不吱声: 1: 我
      发现是 BGR 和 RGB 搞反了: 2: 我
      纬度是超参数 冷启动也影响成功率: 1: 我
    section 祛魅期
      读完 paper 发现 288Hz 实测 14Hz: 2: 我
      原来大家普遍都是裁缝: 2: 我
      真机 RL 本质是 reward 加权的 BC: 2: 我
      半个具身行业建立在 Qwen 上: 3: 我
    section 和解期
      不存在没有意义的复现: 4: 我
      触觉是残差不是主决策 想通了: 4: 我
      数据质量远比数量重要: 5: 我
      先把 Sim2Real 四个坑补上再上 RL: 4: 我
```
...
quadrantChart
    ...
    quadrant-3 "日常怨念"
    quadrant-4 "内行黑话"
    "半个行业建立在 Qwen 上": [0.72, 0.92]
    "真机采集就是财务诈骗": [0.55, 0.95]
    "空调测试": [0.78, 0.88]
    "扫地机器人够好用了吗": [0.62, 0.85]
    "左脚踩右脚涨估值": [0.35, 0.9]
    "大家普遍都是裁缝": [0.8, 0.72]
    "论文 288Hz 实测 14Hz": [0.9, 0.6]
    "真机 RL 是 reward 加权 BC": [0.95, 0.45]
    "大水池小水管": [0.85, 0.35]
    "不学 L 了": [0.88, 0.5]
    "VLA 帕金森": [0.7, 0.3]
    "柔顺控制棺材板压不住": [0.6, 0.25]
...
```mermaid
timeline
    title 黑话演化史：吐槽重心的迁移（2025 H2 → 2026 H1）
    section 2025 H2 调参期
      抖动与频率 : VLA 帕金森 : 变树懒 : 重新发明低通滤波
    section 2026 Q1 祛魅期
      语言模态存疑 : 不学 L 了 : VLA 等于 VA 加营销 : 长瘤子的 VLM
      工程 trap : 纬度是超参数 : 珍爱生命远离 Piper : loss 0.002 真机寄
    section 2026 Q2 架构质疑期
      裁缝派登场 : 大家普遍都是裁缝 : 从浆糊里抽一点 : 大水池小水管
      跨代重复发明 : 柔顺控制棺材板压不住 : 学计算机的总能重复发现机械
      刷分打假 : 不是兄弟真的假的 : 分布内评测不考验泛化
    section 2026 H1 泡沫期
      资本黑话 : 左脚踩右脚 : 养蛊 : 半个行业建立在 Qwen 上
      数采产业链 : 真机采集就是财务诈骗 : 数采场账本
      验收标准 : 空调测试 : 扫地机器人够好用了吗
```
...
pie showData
    title 93 条黑话的主题分布
    "模型/架构质疑（含裁缝派·不学L派·小脑派）" : 20
    "工程与 Sim2Real 踩坑" : 19
    "学术圈 / paper factory / 重复发明" : 17
    "产业·资本·数采·智驾" : 17
    "科研生活梗" : 9
    "评测打假与验收标准" : 6
    "真机 RL 与 DAgger 真相" : 5
