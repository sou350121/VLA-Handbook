...
> **版本**: v3.12 — 2026-08-28（新增 #230-#251：LeRobot 相机漂移实测 37px/两分钟且事后不可恢复、微调 `input_features` 不刷新导致 24/251 公开 checkpoint 配置自相矛盾（7 个官方）、Python 3.14 全面炸掉 `from_pretrained`、同数据 SmolVLA 打赢 Pi0.5 的真机对照、RDA 数据集审计 CLI、TOWN-VLA prompt-form collapse 92.47%→3.00%、FlashVLA 30Hz 流式解码、UCAG-P 相机中心统一动作空间、LM-X 2 万小时含 1000 小时失败数据、TacForcing 执行期触觉、M3 模态遮蔽 +21.7%、ROS2SmolVLA 工业臂、PredVLA 0.68M 参数、FLARE 失败恢复、InstructMove 文本不可省 benchmark、StreamPI/UniMem/TemporalFlow 记忆三件套、GVLA 多夹爪 10.3 万演示、TrapVLA/CertVLA 安全线，以及 19 条摘要级 arXiv 速查）
> **上版**: v3.11 — 2026-08-21（新增 #211-#229：LeRobot v0.6.1 破坏性变更、Grabette/Gripette 490€ 手持采集器、GR00T 1.7 端到端配方 + SO-101 命令行、GS-VLA 相机位移 90%→10%、ReflexBench 可配置延迟、Embodiment Gap 综述、FACT 接触失败根因、G0.5 单流自回归、GigaBrain-0.7、EcoVLA 端边协同、ViTaR 触觉残差、τ₀-VLA test-time compute、CDPR 零示范 RL 对齐、proprioception 接法消融，以及 4 条深挖的 LeRobot 静默故障 issue）
...
| 230 | 相机在一次 eval 里就漂了 37px，而且事后无法找回 | GitHub Issue (huggingface/lerobot #4496 / PR #4495) | 2026-08 | Debug, Data | [链接](https://github.com/huggingface/lerobot/issues/4496) | **给 #215（相机微小位移 → LIBERO 90%→10%）补上的真机现场数据，本期最该照做的一条**。**测量**：SO-101 + 三脚架外部相机，一段**两分钟 eval 录制，全程没有人碰过**；用两个刚性背景地标（显示器边框 + 插线板，均未被手臂遮挡）做模板匹配追踪——frame 0 基准，frame 600 已漂 (+13,−22)px / (+21,−19)px，frame 3000 漂到 (+37,−10)px / (−7,+41)px，**两个地标一致确认相机确实动了，且约在 frame 1800 有一次阶跃**。没人发现，这次 run 被当作策略结果记了分。**更关键的第二个实验：事后找不回来**。想从数据集反推原始位姿——SIFT + RANSAC 单应性，5 张实时帧 × 5 段同房间训练 episode，**25 对全部只有 5–11 个 inlier，没有一对能拟合**。结论：**参考位姿必须提前主动采，事后无解**。**第三个必须记住的点（作者专门论证）**：眼在手上的相机视野是关节角的函数，**不锁手臂位姿去比帧就是在量手臂不是量相机**——同一段未中断 episode 的同两帧，wrist 相机 32 inliers / dx −131px / roll −15.4°，external 相机 142 inliers / 0.0px / 0.00°；锁定位姿后同一个 wrist 相机能拟到 700–1200 inliers、亚像素级。固定相机同样受影响（机器人占画面大时 inlier 集中在机器人身上，手臂旋转会被读成相机 roll）。**提议的工具**：`lerobot-check-cameras --mode=save|check`，参考帧 + 手臂位姿存到 `~/.cache/huggingface/lerobot/calibration/cameras/<robot>/<id>/`，check 直接输出物理纠正方向（`front pan LEFT 25px`）并以非零退出码 gate 录制脚本。**行动项**：在这个 CLI 进主线之前，**自己在每次采集/评测前后各拍一张固定参考帧存档**——成本几秒，省下的是"策略昨天还好好的"那种整周的排查 |
| 231 | 微调不刷新 `input_features`：**251 个公开 checkpoint 里 24 个（9.6%）配置与自带统计量自相矛盾，其中 7 个是官方 `lerobot/` 发布** | GitHub Issue (huggingface/lerobot #4517) | 2026-08 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4517) | **stats 类静默故障的第四次出现（#6 #30 #227 之后），但这次量化到了生态规模**。**根因（`factory.py` L333-335）**：`make_policy` **无条件**用数据集刷新 `output_features`，但 `input_features` **只在为空时才刷新**；而从预训练 checkpoint 载入的 config 的 `input_features` 永远非空——于是**跨本体微调会永久保留基座模型的 feature spec，同时归一化统计量却来自新数据集**。两者不对称，看起来是意外而非设计。**自相矛盾的官方证据**：`lerobot/smolvla_libero`（其 `train_config.json` 自述从 `smolvla_base` 在 `lerobot/libero` 上微调而来）——`config.json` 写 `observation.state` shape **[6]** + camera1/2/3，而同仓库的 normalizer safetensors 是 **(8,)**、rename 步骤只产出 camera1/camera2，训练数据集 `lerobot/libero` 是 **[8]** + 2 个图像 key。**[6] + 三相机是 smolvla_base 的 SO-100 特征，原封不动带过来了**。模型本身能跑（SmolVLA 把 state pad 到 `max_state_dim`、只遍历实际存在的图像），**但任何信任 `config.json` 的东西——集成代码、config 驱动的 eval、model card——读到的都是错的规格**。**这是 #2418「微调能跑但 eval 张量 shape 不匹配」的根因**，那个帖里社区摸索出的 workaround（手动传 `--policy.input_features`）恰好就是 L334 跳过的那次刷新。**生态扫描（作者用 checkpoint linter 扫了下载量前 300 的 lerobot 标签模型仓）**：**24/251 个可验证 checkpoint（9.6%）确证中招**；**7 个是官方发布**（合计约 3.85 万次下载）：`smolvla_libero`(8)、`smolvla_robotwin`(14)、`smolvla_libero_plus`(8)、`smolvla_metaworld`(4)、`smolvla_robocasa`(16)、`smolvla_vlabench`(7)、`smolvla_robocerebra`(8)——**全部声称 smolvla_base 的 state [6] + 3 相机**；针对 `smolvla_base` 微调这一机制的中招率是 **39%（19/49）**；样本里甚至出现**二代传播**（一个 stale 微调的 stale 微调）；另有 20 个 checkpoint 声明了其 preprocessor rename map 根本不会产生的**幽灵相机**。**自检**：`uvx lerobot-dataset-lint <repo>` 现在会自动识别 checkpoint。**行动项**：**跨本体微调时显式传 `--policy.input_features`；集成任何第三方 checkpoint 前，先拿 normalizer 的统计量维度去对 `config.json`，不要信 config** |
| 232 | Python 3.14 上 LeRobot 完全不可用：所有 `from_pretrained` 在解析器构造阶段就 TypeError | GitHub Issue (huggingface/lerobot #4509 → PR #4510) | 2026-08 | Debug | [链接](https://github.com/huggingface/lerobot/issues/4509) | **新装环境炸掉的高频原因，而且 CI 抓不到**。**现象**：`pyproject.toml` 写的是 `requires-python = ">=3.12"`，所以 pip/uv 会痛快地在 3.14 上装上 LeRobot；然后**每一条构造 config 解析器的代码路径立刻失败**：`TypeError: typing.Dict[str, lerobot.configs.types.PolicyFeature] | None is not callable`。**报错发生在 `ArgumentParser` 构造时、任何参数被解析之前，调用方无法捕获恢复**；`PreTrainedConfig.from_pretrained` 以及所有 `Policy.from_pretrained` 全部受影响，draccus 驱动的 CLI 入口同样。**根因不在 LeRobot**：Python 3.14 让 `argparse.add_argument` 开始校验 `type=` 是否 callable（之前接受任意值、只在转换时才失败），而 draccus 把原始字段注解直接当 `type=` 传进去——**任何 union 类型的 dataclass 字段都会炸**，`Optional[str]` 就够了。在 `PreTrainedConfig` 上是 `input_features` / `output_features`（`dict[str, PolicyFeature] | None`）。上游 `marin-community/draccus#69` 仍未发版修复（该仓 6 月后就没动静）。**CI 为什么没抓到**：所有 workflow 都 pin 了 `PYTHON_VERSION: "3.12"`，没有任何 job 跑 3.13/3.14。**为什么会撞上**：**Homebrew 现在的默认 `python3` 已经是 3.14**，macOS 新机第一次 `from_pretrained` 就中。**行动项**：装 LeRobot 前先 `python -V`，用 **3.12/3.13**；已经中招的不要往下查自己的代码，直接换 Python 版本 |
| 233 | 同一份 50 episodes 不完美真机数据：SmolVLA 450M 全量微调**打赢** Pi0.5 3B，且训练 loss 最低的那个部署最差 | GitHub Issue (huggingface/lerobot #4463) | 2026-08 | Recipe, Arch | [链接](https://github.com/huggingface/lerobot/issues/4463) | **一个被完整记录的受控对照现场，直接反驳"模型越大越好"**。**设置**：REALMAN NERO 双臂，任务"把黑海绵放进盒子"（左臂摆盒子、右臂放海绵），**同一份 50 episodes 遥操数据、同一平台、同一条指令**，chunk_size=50。数据作者自陈的缺陷：右臂抓到海绵之后「移动到盒子上方 → 微调位置 → 松手」三个阶段**连得太紧、缺少中间停顿和稳定的视觉观测**，观测-动作对齐不理想。**配置**：SmolVLA 全量微调（~450M，backbone 不冻，batch 32，20k steps）；Pi0.5 全量微调（~3B+，backbone 不冻，batch 64，10k steps）；两者另各跑一个"只训 action expert（冻结 backbone）"版本。**结果**：**SmolVLA 全量微调最好**——执行稳定、多次完成任务、场景被轻微扰动后还能恢复；**Pi0.5 全量微调训练 loss 最低，但部署更差**——场景被扰动时预测动作与视觉输入脱节，**机器人倾向于沿着学过的动作序列走而不是响应当前观测**；两个 action-expert-only 版本都大幅退化，Pi0.5 的那个几乎完全失败。**但反过来的那一半同样重要**：改写语言指令做泛化测试时，**Pi0.5 全量微调的语言泛化明显更强**，SmolVLA 的 action-expert 版几乎全废。**可抄的结论**：(1) 又一次印证 **loss 不反映真实成功率**（#16 #209）；(2) **数据不完美时，大模型不一定是更优解**——容量更大反而更容易把 suboptimal 的连续动作序列背下来（与 #13 OpenVLA-OFT「L1 的有限表达力反而起正则作用」是同一个道理的两个侧面）；(3) **选型要分开看"执行鲁棒性"和"语言泛化"两个指标**，这个案例里它们指向相反的模型；(4) 想省算力只训 action expert 的，先看这条——两个模型上都掉得很厉害。⚠️ 单人单任务 n=1 案例，无成功率数字，只有定性描述与视频 |
| 234 | 数据集质量终于有了可跑的审计 CLI：同一台 xArm，两个官方数据集 idle 比例差 4 倍 | GitHub Issue (huggingface/lerobot #4513) | 2026-08-25 | Data | [链接](https://github.com/huggingface/lerobot/issues/4513) | **#10「OXE 数据质量差但没人量化」的另一条落地路径（工具向，与 #209 Robometer 的模型向互补）**。**工具**：`robot-data-audit`（RDA v0.5.4），`pip install robot-data-audit` 后 `rda audit <local_dir>`，**全离线、默认阈值**。**结果（2026-08-25 从 HF Hub 全新 snapshot_download，三个数据集的完整性层——NaN/Inf、时间戳合法性、缺帧、schema——全部通过）**：`xarm_lift_medium` 800 eps，767 PASS / 33 REVIEW，动作尖峰 6 处（5 eps），**中位 idle 20.8%**；`xarm_push_medium` 800 eps，238 PASS / **562 REVIEW**，尖峰 845 处（集中在 500 eps），**中位 idle 83.3%**；`jaco_play` 1,085 eps，390 / 695，尖峰 11,958 处（837 eps），**中位 idle 74.1%**。**值得记住的观察**：lift 与 push 是**同一台 xArm 平台**，却落在量表两端，**idle 差约 4 倍**。作者自己的解释偏向任务差异（举方块 vs 推平面物体需要持续接触）而非采集马虎，并明确说"REVIEW 不等于数据坏了"。**对读者的实操含义**：**高 idle 不是 bug，但它是训练前必须知道的一个属性**——loss 会偏向"什么都不做"，可能需要课程学习或样本加权；用公开数据集前先跑一遍审计，比训完发现策略学会站着不动便宜得多 |
| 235 | TOWN-VLA：往 prompt 后面**追加任何文本**，成功率从 92.47% 崩到 3.00%——包括无意义的等长填充 | arXiv 2608.23224 | 2026-08-24 | Debug, Arch | [链接](https://arxiv.org/abs/2608.23224) | **本期第二个"数字最吓人"的发现，且矛头直指所有 RAG/检索增强 VLA 的做法**。**审计结果**：在配平的对照中，**把检索到的原始文本直接追加进执行 prompt，平均成功率从 92.47% 掉到 3.00%**；而且**有意义的追加与长度配平的无意义追加在全部 500 个状态上都失败**——作者把这命名为 **prompt-form collapse：改变指令的形式（而非增加有用语义）本身就足以主导执行结果**。**方法（TOWN-VLA = Think Only When Needed）**：把「生成候选」与「有没有权限改动策略输入」分开——一条固定的兼容性规则只授权规范化的紧凑指令，否则**逐字节还原原始 Base prompt**。900 条审计路由全部遵守该契约（525 条哈希级还原 Base，375 条授权 prompt 保持任务签名）。**数字**：配平的 4×7 LIBERO-Plus 评测、每方法 10,030 episodes，成功率 **69.5% → 73.1%**（+362 episodes，95% CI 1.89–5.45pp），六个扰动轴与四个 suite 全面改善；真机 PiPER 臂 + 冻结 π0.5 checkpoint，每方法 150 次试验，**52.7% → 78.7%（p=3.16×10⁻⁶）**。**对读者的含义**：**想给冻结的 VLA 加检索/加提示词，默认做法（拼接到 prompt）是负收益且幅度巨大**——必须有"什么情况下才允许改动 prompt"的显式门控。同组另一篇 #241 Pointing-VLA 用了同一套真机基线（52.7%） |
| 236 | FlashVLA：流式动作解码，单卡 ≥30Hz 且异步执行天然连续 | arXiv 2608.27384 | 2026-08-27 | Edge, Arch | [链接](https://arxiv.org/abs/2608.27384) | **flow-matching VLA 提速路线上的又一条正交解法（对照 #174 SnapFlow 蒸馏、#197 FASTER 改架构、#206 FLASH 投机解码、#203/#204 换运行时）**。**痛点**：现有方法要么降延迟、要么减少异步执行的空转，**很少能同时做到低延迟 + 准确且时序一致的异步执行**。**机制**：维护一个**含多个不同噪声水平 chunk 的流式动作缓冲区**，用 **chunk-wise causal attention** 解码——**每次推理步产出一个可执行 action chunk**；其 chunk 级自回归形式**隐式保住了动作连续性**，因此**不需要额外的 future-state conditioning 就能平滑异步执行**（对照 #15 ACTSmooth 需要 prefix conditioning + relative action 才拿到平滑）。**数字**：真机部署下**单 GPU ≥30Hz 控制频率**，仿真与真机实验中大幅提速且任务性能保持。17 页 8 图 |
| 237 | UCAG-P（小米）：不做动作重定向，改用**相机可观测的锚点运动**统一异构本体 | arXiv 2608.26058 | 2026-08-26 | Arch, Data | [链接](https://arxiv.org/abs/2608.26058) | **跨本体统一动作空间的第三种答案（对照 #200 LingBot 55D、#205 RhinoVLA 72D 的"统一 slot + mask + per-robot LoRA"范式）**。**论点**：现有做法靠显式动作重定向、人到机器人视频合成、或每数据集一个适配分支，**从根本上阻碍统一策略的联合学习**。**方法**：不把机器人专属指令当共享策略目标，而是用**图像与相机坐标系下的锚点运动**来表示操作——**机械臂、人形、人手都被当作同一套动作 schema 的不同本体**；再用一个几何条件化的 action translator 把预测运动与目标本体运动学结合，产出可执行控制。解耦架构让共享策略学"可迁移的操作几何"，同时保留本体特定的可控性。**数据**：**4.03K 小时机器人 + 仿真数据 + 2.34K 小时人类演示**（比 #213 GR00T 1.7 的 32K+8K、#220 GigaBrain 的 37K、#223 τ₀ 的 40K 小一个量级，是本条主张的重点——**表示对了可以省数据**）。**单一 checkpoint、无 benchmark 专属微调**：LIBERO 98.3%、RoboTwin Easy/Hard 88.7%/89.2%、**LIBERO-Plus 零样本 82.0%**、RoboCasa GR-1 62.0% |
| 238 | LM-X：**训练数据里放了 1,000+ 小时失败 rollout**，RoboTwin2.0 randomized-hard 74.1% vs GR00T N1.7 55.4% | arXiv 2608.25757v2 | 2026-08-26 | Arch, Data | [链接](https://arxiv.org/abs/2608.25757) | **三个值得单独记住的工程决定**。**(1) 显式多时间尺度预测头**：在线发出三个被显式监督的信号并直接条件化动作生成——**RTG（return-to-go，可见任务进度）**、**ETG（event-to-go，下一个语义转换点）**、**异方差 action flow（用传播方差估计局部可靠性）**；因此可解释性是**控制的内在部分而非事后生成**（与 #209 Robometer 把 progress 做成外部奖励模型形成对照）。**(2) 花大钱之前先跑门槛实验**：在 **64 张 NVIDIA B200 上 20 天的预训练**之前，先做一个受控的五任务预训练 gate 验证设计——完整模型比 action-only backbone 高 **16.0 个点**、比最强单头变体高 **10.8 个点**，验证通过才开跑。**这个"先 gate 再烧算力"的纪律值得任何要做大规模预训练的团队照抄**。**(3) 数据配方**：**2 万+ 小时真机轨迹，其中包含 1,000+ 小时失败的策略 rollout**——呼应 #11 π*0.6「纯模仿的 compounding error 需要 recovery 数据」。**数字**：50 个 RoboTwin2.0 randomized-hard 任务 **74.1%（GR00T N1.7 为 55.4%）**；7 个真机任务 **68.6% vs 50.7%**。定性观察：RTG 能跟踪语义进度与可见的倒退，**方差在犹豫和振荡控制时升高**——这可以直接当作真机的在线异常信号 |
| 239 | TacForcing：不要另外挂高频控制器，把触觉直接接进**流式** action expert | arXiv 2608.25798 | 2026-08-26 | Arch | [链接](https://arxiv.org/abs/2608.25798) | **与 #222 ViTaR 是同一问题的两条不同路线，值得对照选型**。**痛点定义得很准**：chunk 式 VLA 从**执行开始之前**采集的观测预测整个 chunk，于是**触觉条件在执行过程中一直是过期的**；而接触状态恰恰在一个 action horizon 内就会显著演化。现有触觉反应式方法通常另配一个高频控制器，架构与训练复杂度都上去了。**方法**：**用流式 action expert 替换标准 action expert**，按执行期间不断到来的触觉观测生成动作；再加 **EATA（Execution-Aware Tactile Attention）**——**把触觉条件化限制在"即将被执行"的那些动作上**，压缩触觉采集与动作执行之间的时间错位。**数字**：6 个 UniVTAC 仿真任务平均 **65%**，3 个真机接触密集任务 **69%**，均优于强基线。**选型对照**：想给**已有的冻结 VLA** 加触觉且不想重训 → #222 ViTaR（有界残差调制，+30.6pp）；愿意改 action expert 换取执行期实时触觉 → 本条。两者与 #218 FACT（先分清是精度失败还是力失败）构成本季触觉三件套 |
| 240 | M3：训练期随机遮蔽模态通道，双臂任务 +21.7%，**零架构改动、零预训练** | arXiv 2608.22419 | 2026-08-23 | Recipe | [链接](https://arxiv.org/abs/2608.22419) | **本期性价比最高的一条 recipe——改训练循环几行，不改模型、不需要大规模机器人预训练**。**动机观察**：query-based VLA 推理延迟低、适合双臂，但在复杂双臂任务上仍会出现**动作不连续与执行失败**，作者观察到这类失败**常伴随注意力扩散到干扰区域**，猜测多视角 + 语言融合不稳定是成因之一。**方法（Modality Masking Mechanism）**：训练时**随机遮蔽模态通道的子集**，让策略暴露在受控的部分观测下，从而**少依赖干扰性线索、多依赖始终可靠的证据**。仅训练期生效。**数字（相对 Adapter 基线）**：RoboTwin 2.0 十个双臂任务，Clean 设定 **+21.7%**、Clean2Rand（干净数据训练、随机化场景评测）**+11.4%**；**三个长 horizon 真机任务的整任务成功率 +30% 以上**。35 页 22 图 9 表 |
| 241 | Pointing-VLA：把空间 grounding 从"文本坐标"改成**带类型的隐状态读出头**，真机 52.7% → 80.7% | arXiv 2608.23138 | 2026-08-24 | Arch, Edge | [链接](https://arxiv.org/abs/2608.23138) | **接口层的改动，提速与提准同时拿到**。**痛点**：VLA 常把空间 grounding 通过**自回归文本坐标**或不透明的 action token 暴露出来，在多模态推理与机器人执行之间形成脆弱接口。**方法**：基于 Embodied-R1 加一组**几何专用头**，直接预测归一化点、object-functional grounding (OFG) 热力图与视觉轨迹，**不把几何序列化成文本**；执行契约把 **PICK 指派给 source-conditioned OFG、PLACE 指派给 Pointing**，给出阶段对齐的空间目标。**数字**：Bridge/WidowX 四任务集平均 **72.9% SOTA**（无 Bridge 专属微调，CuRobo 开碰撞执行）；OFG/contact 读出**可迁移到 NORA-1.5**，在保持或提升成功率的同时**把记录的控制器时间降低 20 倍以上**；带类型的头比 Embodied-R1 的文本解码**快 6.68–6.90 倍**；作为空间引导接进 π0.5 动作策略时，三个视觉场景下真机自主成功率 **52.7% → 80.7%**。**对读者的含义**：**"让 VLM 用文本吐坐标"是延迟与鲁棒性的双重税**，换成结构化读出头是低风险的工程改动 |
| 242 | ROS2SmolVLA：把 SmolVLA 接到 UR10e 工业臂，**开源 ROS 2 接口** | arXiv 2608.23320 | 2026-08-24 | Edge, Recipe | [链接](https://arxiv.org/abs/2608.23320) | **社区一直缺的那一块：从 SO-101 玩具臂跨到工业级硬件**。**动机**：现有研究要么用**本地跑不动的大模型**（带来合规与安全问题），要么用**实验室级机器人硬件**（掩盖了真实工业场景的落地难度）。**做了什么**：把 HF 的 SmolVLA 适配到 Universal Robots 轻型臂，开源 `ROS2SmolVLA` 仓库实现 **ROS 2 → SmolVLA 的接口**，并给出实现指引；用 **UR10e** 的 pick-and-place 任务验证。**结论**：**SmolVLA 适合需要本地（on-premise）计算的小型任务**。**与 #4368（LeRobot × ROS 2 官方方向 RFC）配合看**——ROS 2 集成是 2026 下半年 VLA 产品化的公共瓶颈。会议版：8th International Conference on Industry of the Future and Smart Manufacturing (Padua & Venice, 2026) |
| 243 | PredVLA：**0.68M 可训练参数**、无机器人数据预训练，LIBERO 短程三 suite 86.9% | arXiv 2608.26673 | 2026-08-27 | Arch | [链接](https://arxiv.org/abs/2608.26673) | **对"到底需要多大模型"的一次极端压力测试（比 #29 VLA-0-Smol 的 500M 又小三个数量级）**。**架构**：语言条件化的**预测编码（predictive coding）**策略，分层生成式递归动力学预测视觉特征与本体感知；**观测只通过由此产生的感知预测误差、经在线推断来影响潜状态**。**数字**：LIBERO 三个短程 suite 平均 **86.9%**，含长程 suite 为 **75.4%**；在**同一冻结前端、同一演示数据、同一动作解码器、同一评测协议**的受控对照下，达到参数量配平的 Transformer 与 LSTM 策略的 **3.7 倍 / 7.4 倍**成功率。**一个漂亮的副产品**：因为观测只通过预测误差影响递归状态，**关掉这条推断路径就得到精确的开环控制条件**——观测驱动修正的贡献变得可直接测量。**对读者的含义**：LIBERO 的高分主要测记忆力这件事（#29 #180）在这里再次被侧面证实；反过来，**小团队在受限任务上未必需要 VLA 规模的模型** |
| 244 | FLARE：给 VLA 装 "Retry / Reset" 两级失败恢复 | arXiv 2608.26645 (CVPR 2026) | 2026-08-27 | Recipe | [链接](https://arxiv.org/abs/2608.26645) | **失败恢复从"多采 recovery 数据"走向结构化方案**。**痛点**：VLA 通常只在**轨迹单调、无失败**的演示上训练，这种对"完美数据"的依赖使其无法从漏抓、掉件、意外碰撞中恢复。**两级机制**：**(1) Retry**——在演示中注入**扰动段与桥接段**，把机器人位姿与环境状态解耦，让策略自主处理执行偏差；**(2) Reset**——针对破坏状态的 OOD 失败，用 **MLLM 离线分析执行视频自动识别 OOD 状态**，据此**定向采集一小批以物体为中心的 "Reset" 技能**，训练成把环境恢复到任务可继续的状态。推理时由**在线 MLLM 监控器在任务执行与 Reset 技能之间仲裁**。接触密集任务上任务成功率与鲁棒性显著提升。**与 #11 π*0.6（coaching + RL 修正）、#3 的「20% recovery episodes 提升成功率」是同一主题的三种做法**：加数据 / 加 RL / 加结构化恢复技能 |
| 245 | InstructMove：一个"**文本不可省**"的指令跟随 benchmark——现有 benchmark 允许策略不读指令就成功 | arXiv 2608.22990 (Horizon Robotics) | 2026-08-24 | Arch, Strategy | [链接](https://arxiv.org/abs/2608.22990) | **继 #180 COIN、#193/#194 之后又一次对评测有效性的攻击，这次针对的是"语言"这一维**。**论点**：判断通用性的关键测试是策略**是否真的在跟随语言指令**，但许多操作 benchmark 让这件事**无法判定**——目标物体或位置往往**视觉上最显眼或是唯一可行的**，策略不 grounding 指令也能成功。**提出的原则（text-indispensable）**：**多个动作在视觉与物理上都应当可行，而只有一个与语言指令一致**。**benchmark 构成**：带语义干扰物的 pick-and-place 场景，把指令跟随拆成**类别识别 / 属性辨别 / 空间推理 / 组合式 pick-and-place** 四类；提供 train-eval 协议与语言依赖度诊断。**实验结论**：代表性 VLA 策略上，InstructMove 能**受控地诊断出视觉捷径**；且 **InstructMove 的仿真数据能提升真机指令跟随表现**。代码在 `HorizonRobotics/RoboOrchardSim`。**对读者的含义**：报告 VLA 的"语言泛化"时，先确认你的评测里语言是不是可以被忽略——#233 那个 SmolVLA vs Pi0.5 的语言泛化对照就需要这类设计才站得住 |
| 246 | StreamPI：**不加任何参数**把单帧 π0.5 变成流式时序模型 | arXiv 2608.26067 | 2026-08-26 | Arch | [链接](https://arxiv.org/abs/2608.26067) | **与 #225（proprioception 接法消融）、#250 TemporalFlow-VLA 同属本期"历史信息怎么接"主题**。**痛点**：π0.5 这类 SOTA 模型在**单帧范式**下运行，无法保留过往观测、空间感知也受限。**方法**：**指令锚定的时序建模**——把每个（视觉观测, 语言指令）对当作一个**原子时间单元**，对内双向注意力做跨模态融合、对间因果注意力保持自回归流式推理，**让语言指令在整个任务执行过程中充当持续的语义锚点**。**最实用的一条工程细节**：为弥合**同步训练与异步真机部署**的差距，提出**随机间隔流式训练**——取一个合适的帧间间隔（如每 3 帧）能让动作执行更快更平滑，**再把间隔随机化则进一步提升对帧时序扰动的鲁棒性**，这正是异步部署所需（呼应 #14 Helix 训练时注入 temporal offset 模拟推理延迟）。借助 LLM backbone 的长度外推能力，**可直接继承预训练单帧权重**，并同时支持单帧与多帧推理。真机（记忆依赖与精细感知场景）与 LIBERO 上均超过 π0.5 |
| 247 | GVLA + MiGA：**103,000 条演示、五种夹爪**——"夹爪不变性"这个隐含假设第一次被正面处理 | arXiv 2608.24603 | 2026-08-25 | Data, Arch | [链接](https://arxiv.org/abs/2608.24603) | **一个几乎所有人都默认、却从没人管的设计位（与 #225 proprioception 接法属同一类"被忽略的默认值"）**。**论点**：现有 VLA **隐含假设夹爪不变**，但抓取策略本质上是本体相关的——**平行夹爪与吸盘要达成同一个抓取目标通常需要完全不同的交互策略**；而现有 VLA 数据集**压倒性地依赖平行夹爪**，根本没有夹爪感知的学习信号。**数据集 MiGA**：跨多台机器人的**五种不同夹爪类型、103,000 条演示**，**显式记录同一任务目标下的策略分歧**。**方法 GVLA**：新的多夹爪 tokenizer + 基于 adapter 的策略路由；夹爪编码诱导出的结构化嵌入**在参数共享与策略分化之间取平衡**，逐层探测确认学到了有意义的夹爪条件化表征。仿真与真机均超基线，并改善对新物体/未见任务的零样本泛化与少样本适配，**换夹爪的适配效率也更高**。**对读者的含义**：换夹爪（尤其平行↔吸盘）时不要指望现有 checkpoint 直接迁移；社区夹爪生态（#154 平行夹爪、#95 Arpeggio）现在有了对应的模型侧答案 |
| 248 | UniMem：把记忆与控制统一进一个 backbone，硬件任务 80.0% vs 分层基线 43.5% | arXiv 2608.22869 (Stanford, Mac Schwager 组) | 2026-08-24 | Arch | [链接](https://arxiv.org/abs/2608.22869) | **针对 #7（π0 memoryless 导致多步任务失败）这个老问题的一个干净解法**。**论点**：现有记忆方案通常**再挂一个 VLM 管长期记忆**，带来记忆瓶颈与割裂的训练管线；而**条件化多个历史帧**虽能提供更丰富的过去场景特征，**但若按任意固定间隔取帧反而会掉性能**（与 #246 StreamPI 的随机间隔结论互为补充）。**方法**：单 backbone 统一高层多模态记忆与低层控制——**事件分类器**决定何时更新记忆、**关键帧编码器**提供稠密空间记忆、**关键帧缓存**压低 rollout 期开销。**数字**：五个仿真 + 四个硬件任务（针对序列记忆与空间记忆），仿真 **93.4% vs 固定间隔取帧基线 68.2%**，硬件 **80.0% vs 分层基线 43.5%**，且推理更快、训练管线更简单。项目页 `losterberg3.github.io/unimem-vla` |
| 249 | 本期 VLA 安全面首次成规模出现：TrapVLA（可配置失败模式后门）+ CertVLA（认证防御） | arXiv 2608.26578 / 2608.20791 | 2026-08-21～27 | Strategy | [链接](https://arxiv.org/abs/2608.26578) | **一个值得放进"6 个月后会后悔忽视什么"清单的新方向**。**TrapVLA（攻击面）**：提出 **Configured Failure Trapping**——不同于既往"任何任务失败都算攻击成功"的后门工作，**要求攻击者控制机器人「怎么失败」**（例如让抓取带一个指定的位置偏移），因此**更难、也更难被检测**；配套数据引擎合成目标轨迹、自动化套件度量 configured-failure 保真度，并建了 **Trap-LIBERO / Trap-RoboTwin** 两个 benchmark 覆盖四种失败模式。实验显示可在**基本不影响干净数据表现**的前提下注入配置好的失败模式（仿真 + 真机）。**CertVLA（防御面）**：针对有界 patch 与纹理攻击的**闭环 VLA 认证防御**——既有的认证 patch 防御针对离散标签，**无法直接认证连续、时序相关的动作**；CertVLA 提出行为一致动作的校准区域 + 确定性覆盖掩码保证至少一次被检查的预测无攻击，并把 query 级判定接成完整闭环 rollout 的认证。**对读者的含义**：**VLA 权重与数据集正在像 LLM 一样被当作供应链**——#231 已经证明生态里 9.6% 的公开 checkpoint 带着自相矛盾的配置在传播，这条线上的恶意版本只是时间问题。**下载第三方 checkpoint 时，行为审计（干净数据表现正常 ≠ 安全）应当进入流程** |
| 250 | TemporalFlow-VLA：用**机器人-表面时序流**做训练期几何监督，部署时完全不用 | arXiv 2608.26821 | 2026-08-27 | Arch | [链接](https://arxiv.org/abs/2608.26821) | **"加历史帧"不等于"学到最近发生了什么"**。**论点**：多阶段操作里**视觉上相似的状态会因为之前执行了什么而需要不同动作**，单纯堆历史帧不可靠。**方法**：用**记录的机器人状态、机器人几何与标定过的相机**构造 **robot-surface temporal flow 作为纯训练期目标**，监督两个执行对齐的时序 query，把结构化历史喂给 action expert；**几何监督路径在部署时不参与计算**。**数字**：LIBERO 平均 **97.63 ± 0.26%**（LIBERO Long **96.60 ± 0.87%**），12 个 RoboTwin 任务 Clean/Randomized **85.5% / 84.2%**，在更长 horizon、多阶段任务上优势最明显。**受控的历史干预实验证明动作预测同时依赖历史内容与时间顺序**。**部署侧**：配合异步特征缓存，时序条件化**保持单帧级的服务端采样延迟**，无额外历史编码开销。同组的 #251 V-Link 用了相同的作者班底 |
| 251 | V-Link：action expert 其实**看不到 VLM 里的几何与语义特征**，补回来后 LIBERO-Plus +31.2% | arXiv 2608.25308 | 2026-08-26 | Arch | [链接](https://arxiv.org/abs/2608.25308) | **一个结构性诊断，解释了大量"泛化不动"的现象**。**发现**：VLA 架构中 **action expert 对 VLM 特征里的 3D 几何与 2D 语义信息访问受限**——这个可达性缺口削弱感知 grounding，限制精细操作表现。（与 #219 G0.5「VLM 沦为 context encoder」是同一批判的两种表述，但 G0.5 的解法是取消独立 action expert，本条的解法是修补通路。）**方法**：在 VL→A 特征传递过程中显式恢复视觉表征——在 VLM 内学习互补的 **Spatial Query 与 Semantic Query**，经**非对称通路**注入 Action DiT：Semantic Query 补充原始 VLM 图像 token，Spatial Query 为空间 grounding 的动作生成提供专用几何条件。**数字（相对基座 GR00T N1.6）**：LIBERO **+1.9%**、**LIBERO-Plus +31.2%**、RoboTwin 2.0 **+18.8%**；AGIBOT A3 Ultra 上两个真机人形任务 **+20% / +24%**。**读法**：LIBERO 只 +1.9% 而 LIBERO-Plus +31.2%，**再次说明 LIBERO 已饱和、增益只在扰动/泛化维度上才看得见**（#29 #180 #215） |

---

## 本期 arXiv 速查（2026-08-20 → 2026-08-27 窗口，仅核对到摘要）

> **说明**：下列条目**只读了 arXiv 摘要，未展开正文核对数字**，用途是「知道有这个方向存在」的速查表，引用前请自行打开原文。
> 上面 #235–#251 的数字同样取自摘要（arXiv 惯例是摘要数字与正文一致，但未逐条交叉验证）。

| arXiv | 标题 | 一句话为什么值得看 |
|-------|------|------------------|
| [2608.24101](https://arxiv.org/abs/2608.24101) | TrAct: Bridging Robot Control and Visual Prediction with Visual Tracks | 用**视觉轨迹**当控制与世界模型之间的本体无关接口；LIBERO-INTEGRAL 27%→55%、真机 Franka 49%→76%（基线 π0.5） |
| [2608.25659](https://arxiv.org/abs/2608.25659) | GaussianDream++: Efficient 3D Gaussian World Modeling | 训练期插 20 个 world token 做 3D 高斯监督，**推理时全部移除**；LIBERO 98.6% / LIBERO-Plus 87.8%，真机 29.2%→52.5%（对复现的 π0.5） |
| [2608.23478](https://arxiv.org/abs/2608.23478) | Act with Intent (INDI): Distilling Behavior Intent | 冻结教师 VLM 蒸馏"行为意图"进 action decoder；SimplerEnv-Bridge 把 GR00T-N1.7 从 64.3% 提到 84.7% |
| [2608.21740](https://arxiv.org/abs/2608.21740) | CounterAlign: Counterfactual Supervision | **只用成功演示**造反事实负样本（指令错配重标注）做离线 RL 奖励模型，不需额外 rollout；LIBERO-PRO 鲁棒性提升 |
| [2608.19891](https://arxiv.org/abs/2608.19891) | EXIMO: VLM Guided Exploration of VLA Policies (DeepMind) | explore（VLM 当 planner 拆长程任务并采编排数据）→ imitate → optimize（残差 off-policy RL）三段式微调 |
| [2608.20735](https://arxiv.org/abs/2608.20735) | ForeTime-VLA: Causal Future-Token Distillation | 从冻结 Fast-WAM 教师蒸馏未来 token，**部署时不跑世界模型**；传送带抓取静止 81.1% / 慢速 58.9%，三档速度 44/90 vs π0.5 23/90 |
| [2608.21247](https://arxiv.org/abs/2608.21247) | Action-JND: Token Compression via Just Noticeable Difference | 压缩判据从"视觉冗余"改成"**这个 token 变多少才会让动作偏出容忍带**"；OpenVLA / OpenVLA-OFT 上激进压缩比下更可靠 |
| [2608.25585](https://arxiv.org/abs/2608.25585) | RA-VLA: Retrieval-Augmented VLA for Test-Time Adaptation (ICML 2026) | 检索增强的免训练适配；**与 #235 TOWN-VLA 的 prompt-form collapse 结论直接冲突，值得对读** |
| [2608.19589](https://arxiv.org/abs/2608.19589) | OrthoSkillVLA: Continual Skill Learning (PRCV 2026) | 持续学新技能不灾难性遗忘：**VLM 与 ActionHead 分开加子空间约束** + 输出层轻量 MoE 解码器 + 免训练路由 |
| [2608.25864](https://arxiv.org/abs/2608.25864) | MA-VLA: Multi-Arm VLA (ECCV 2026) | 多臂协作：中层原子 prompt 分配到各臂 + **Arm Shuffle**（训练期打乱臂的角色）实现未见协作模式的组合泛化 |
| [2608.24959](https://arxiv.org/abs/2608.24959) | GaussVLA (BMVC 2026) | Mamba 底座 + 3D 高斯空间 token + 深度感知 CoT；**200M 参数** LIBERO 平均 93.5%、Spatial suite 100% |
| [2608.24115](https://arxiv.org/abs/2608.24115) | PonderPounce: MLLM as Episode Context Engine | 复用 MLLM 原生因果上下文当机器人记忆（S2 Ponder + S1 Pounce）；**认知刷新 p50 78ms / 动作调用 25ms，支撑 20Hz** |
| [2608.27079](https://arxiv.org/abs/2608.27079) | GRAFT: Grounded Online RL Adaptation | 生物医学操作场景的在线 VLA 适配；区域级监督学视角专属视觉锚点 + **缓存 VL prefix 复用**加速在线学习，+25pp |
| [2608.26788](https://arxiv.org/abs/2608.26788) | Decoupling Planning and Control (COLM 2026) | VLM planner 出稀疏高层指令 + 世界模型控制器高频执行；**换 planner 无需微调**，7 个环境里 6 个胜过 VLA 基线 |
| [2608.20556](https://arxiv.org/abs/2608.20556) | Logic-VLA: Temporal Logic Conditioned VLA | 推理时喂 **STL（信号时序逻辑）规范**约束安全行为；STL 满足率 +24.8～40.7pp，名义任务成功率最多只掉 1.8pp |
| [2608.21031](https://arxiv.org/abs/2608.21031) | PhysCaP: Physics-Informed Code-as-Policy | **免训练**从本体感知估计物体质量与刚度（不加传感器）；Planner 决定何时停止探索、Prioritizer 排序，解决 VLA"只会被动观察"的盲区 |
| [2608.20743](https://arxiv.org/abs/2608.20743) | Multimodal Speculative Decoding 综述与实证诊断 | #206 FLASH 那条线的系统性综述；含 VLA 架构，回答"块并行生成式 drafting 在多模态上到底行不行" |
| [2608.20478](https://arxiv.org/abs/2608.20478) | EndoLIFT: Bidirectional Endoscopic Control | 医疗内窥镜 VLA；提出 **intent aliasing**（几乎相同的观测需要相反的轴向动作）——任何双向/可逆任务都有这个坑 |
| [2608.25395](https://arxiv.org/abs/2608.25395) | TARCAT: 建筑工种任务分类法 | 从 91 个 O*NET 任务 + 30 段教学视频提炼 **41 个动作原语**；给"VLA 进建筑业需要哪些能力"一个可对照的清单 |
...
*v3.12 — 2026-08-28 更新。Blog 251 条（#1-#251）+ Discord 154 条（D1-D154）+ GitHub 速查 21 条 + 本期 arXiv 速查 19 条，共 445 条。窗口 2026-08-21 → 2026-08-28（一周）。*

***v3.12 三条主线**：*

*1. **「相机」这条线本期从量化走到了工具化**。v3.11 的 #215 GS-VLA 给出「相机微小位移 → LIBERO ~90% 掉到最坏 ~10%」的仿真刻度；本期 #230 补上真机现场：**一段两分钟、全程无人触碰的 eval 录制里，相机就漂了 37px，且约在 frame 1800 有阶跃**——更致命的是作者证明了**事后无法用 SIFT+RANSAC 从数据集反推原始位姿（25 对全部拟合失败）**，参考位姿必须提前主动采。附带一条几乎所有人都会踩的方法论错误：**不锁手臂位姿去比帧，量到的是手臂不是相机**（同一段未中断 episode 里 wrist 相机会读出 −131px / −15.4° 的假漂移）。行动项非常便宜：每次采集/评测前后各存一张固定参考帧。*

*2. **stats/config 类静默故障从「个案」升级为「生态问题」**。#6 #30 #227 #229 是四次独立个案；#231 第一次把它扫成了数字——**下载量前 300 的 lerobot 模型仓中，251 个可验证 checkpoint 里有 24 个（9.6%）的 `config.json` 与自己仓库里的 normalizer 统计量互相矛盾，其中 7 个是官方 `lerobot/` 发布（约 3.85 万次下载）**，针对 `smolvla_base` 微调这一条路径的中招率高达 **39%（19/49）**，且样本中已出现**二代传播**。根因是 `make_policy` 只在 `input_features` 为空时才刷新它。**读者的最小行动**：跨本体微调显式传 `--policy.input_features`；用第三方 checkpoint 前拿 normalizer 维度去对 `config.json`，不要信 config。配套 #232（Python 3.14 让所有 `from_pretrained` 在解析器构造阶段就 TypeError，而 CI 全 pin 在 3.12，Homebrew 默认 python3 已是 3.14）——**本期两条最可能让新装环境无声或有声地废掉的坑**。*

*3. **「更大不一定更好」第一次有了受控的真机现场**。#233：同一份 50 episodes 的不完美真机数据、同一平台、同一指令，**SmolVLA 450M 全量微调打赢 Pi0.5 3B+**，而且**训练 loss 最低的那个（Pi0.5）部署最差**——扰动后它倾向沿着背下来的动作序列走而不是响应观测。但同一案例的反面同样重要：**改写指令做语言泛化测试时 Pi0.5 明显更强**。这与 #13 OpenVLA-OFT「L1 的有限表达力反而起正则作用」是同一机制的两个侧面，并第三次印证 **loss 不反映真实成功率**（#16 #209）。选型时「执行鲁棒性」与「语言泛化」需要分开评，本例中两者指向相反的模型。*

***其他值得单独记住的数字**：#235 TOWN-VLA——**往 prompt 后追加任何文本（含长度配平的无意义填充）都能把成功率从 92.47% 打到 3.00%**，直接质疑所有把检索结果拼进 prompt 的 RAG-VLA 做法；#251 V-Link 在 LIBERO 只 +1.9% 而 LIBERO-Plus +31.2%，再次说明**增益只在扰动维度上才看得见**；#238 LM-X 的工程纪律值得抄——**64×B200 跑 20 天之前先用五任务 gate 验证设计**，且训练数据里主动放了 1,000+ 小时失败 rollout。***

***新出现的方向**：VLA 安全（#249 TrapVLA 可配置失败模式后门 + CertVLA 认证防御）——考虑到 #231 已经证明生态里 9.6% 的公开 checkpoint 带着错误配置在传播，**权重供应链审计应当尽早进入流程**。*

***采集缺口与说明**：(1) **Discord 连续第三期未采集**（Chrome 未登录），下次运行前必须补登录；(2) **Physical Intelligence 博客本期无新文**，已直接核对官网，最新仍为 2026-04-16 π0.7（连续第二期确认）；(3) HuggingFace Blog 的 `?tag=robotics` 过滤本期未生效（返回全站近期文章），本期 HF 侧内容为空，下次需改用 `huggingface.co/blog/<org>` 逐组织路径或 blog 仓库 git log；(4) **#235–#251 与本期 arXiv 速查表的全部数字均取自 arXiv 摘要，未展开正文交叉验证**——投稿或对外引用前请自行打开原文核对（沿用 #225 的标注惯例）；(5) #233 是单人单任务 n=1 的定性案例，无成功率数字。*
