...
> **版本**: v3.10 — 2026-07-31（新增 #198-#210：LeRobot v0.6.0 世界模型策略 + 六大 benchmark + DAgger CLI、官方算力硬件指南、LingBot-VLA 2.0 55D 统一动作空间、Gemini Robotics 2 全身控制、NVIDIA WAM 路线全景、vla.cpp、Embodied.cpp、RhinoVLA token 效率、Realtime-VLA FLASH 投机推理、机器人 leaderboard 刷榜、Figure 03 进 BMW 物流线、Robometer/TOPReward 奖励模型、SmolVLA 微调失败对质）
...
> **本期（2026-07-31）未采集**：Chrome 会话未登录 Discord（跳转 `discord.com/login`），无法执行搜索栏采集。本期社区侧内容改由 GitHub Issues（#210）与实践者博客补位。下次运行前需在 Chrome 中登录 Discord。
