...
> **核心定位**: 用离散扩散世界模型替代真实/仿真执行，解决"在数千环境×数千任务上评估机器人策略不可行"的痛点；相比 WorldEval/WorldGym/Ctrl-World 等视频扩散基线，通过统一 token 空间实现真正的动作可控性。

## ⚡ 快速判斷（30 秒讀完這段就夠了）

| 維度 | 判斷 |
|------|------|
| 核心結論 | 将视觉、语言、动作统一映射为离散 token，用 Masked Discrete Diffusion 联合预测未来观测和任务进度 token，在 LIBERO/RoboTwin/真机上 Pearson r≈0.9 匹配真实执行成功率 |
...
|  动作可控性 | 弱（视觉先常覆盖动作信号，幻觉成功） | 强（self-attention 直接连接动作↔视觉 token） |
...
- 滑动窗口采样最近 K=4 个关键帧（stride=Δ，与动作块长度对齐）
...
- 推理阶段：模型联合输出未来观测 + 进度分数 v̂ ∈ [0,1]，v̂=1 即判定成功
...
| ỹ_{t+Δ} | 被前向过程腐蚀后的目标序列（部分 token 被替换为 [MASK]） |
| λ | 扩散级别，λ ~ U(0,1)，控制掩码比例 |
| m(λ) | 掩码概率（被掩码的 token 数量比例） |
| Ω_λ | 被掩码位置的索引集合 {j | ỹ_j = [MASK]} |
| w_j | 模态重平衡权重：进度 token w=2，视觉 token w=1 |
| p_θ | Transformer 预测的 token 分布 |
...
> 符号与本文保持一致：Δo_t 表示状态转移（差分图像），norm(·) 为 per-sample RMS 归一化。ΔLPIPS 衡量的是"状态变化的感知保真度"而非"绝对状态的相似度"——这正是动作可控性的核心。
...
2. 目标序列（未来观测 o_{t+Δ} + 进度 v_{t+Δ}）：
   - 未来视觉 (256²) → 64 个 token
   - 进度 token → 1 个 token（如 "0.3"，表示 30% 完成）
   - 总目标：65 个 token

3. 前向腐蚀：λ=0.5 → 掩码 50% 的目标 token → 32 个被掩码
...
**推理阶段**（闭环评估策略 π）：
1. 策略 π 输出动作 a_0 → 世界模型生成 ô_Δ + v̂_Δ
2. 若 v̂_Δ < 1 → ô_Δ 成为新 o_t → 策略继续输出 a_Δ → 循环
3. 经过 T 步后，若 v̂_T = 1 → 判定成功；否则失败

**具体数值推演**（假设策略 π_0 在 100 次 rollouts 中）：
- 真实执行成功率：72%
- dWorldEval 自动评估成功率：69%（Pearson r≈0.91 across checkpoints）
- 世界模型幻觉率：<5%（基线 WorldGym 在失败子集上 ΔLPIPS 从 0.347 飙升到 0.650，dWorldEval 保持稳定）
...
| 动作 chunk 长度 | Δ ∈ [2,8] | 短 chunk 响应快但计算开销大；长 chunk 效率高但时间粒度粗 |
...
| 跨域泛化 | sim→real ΔLPIPS ≈ 0.31-0.36 | 统一 token 化对 domain gap 有一定鲁棒性 |

**工程含义**：
- 控制频率：Δ=4 时，每 4 步动作生成一次未来观测，适合 10-20Hz 的控制频率
...
**评测任务设置**：
- RQ1 动作可控性：专家成功子集 D_succ vs 次优失败子集 D_fail，用 ΔLPIPS 度量
...
- RQ5 ΔLPIPS 诊断价值：随机打乱动作验证指标敏感性

## 6. 能力与失败模式 (Capabilities & Failure Modes)

### 能做什么
- **跨平台策略排序**：在 LIBERO/RoboTwin/真机上均达到 r≈0.91-0.93 与真实执行相关（论文 Fig.7b-d）
- **忠实反映失败**：对次优动作不幻觉成功，ΔLPIPS 在失败子集上保持稳定（论文 Table 1）
...
2. **离散化不损失关键信息**：FAST tokenizer 将连续 6-DoF 动作离散化，ΔLPIPS 衡量的是视觉结果而非动作本身的精度。如果离散码本不够细，某些微妙的动作差异可能被抹平。
...
- **值得精讀原文的人**：
  ...
  2. 构建机器人策略 benchmark 的工程团队——ΔLPIPS 指标和进度 token 机制可直接复用
