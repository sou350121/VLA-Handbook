...
如果说 $\pi_{0.5}$ 解决了"去哪儿" (泛化) 的问题，那么 $\pi_{0.6}$ 则解决了"做得好" (熟练度) 的问题。
...
### 1.1 5B VLM Backbone
- **规模**: 从 $\pi_0$ 的 3B 升级到 **5B 参数**。
- **意义**: 更大的模型意味着更强的语义理解能力，能够处理更复杂的长指令 (e.g., "把那个红色的、有点破损的盒子折叠好放进箱子里")。

### 1.2 Action Expert (动作专家)

> **核心创新**: Action Expert 是 $\pi_{0.6}$ 最重要的架构升级，解决了 VLM "脑子聪明但手笨" 的问题。
...
| 特性 | $\pi_0$ (Flow Matching) | $\pi_{0.6}$ (Action Expert) |
| :--- | :--- | :--- |
...
## 2. $\pi^*_{0.6}$ (Pi-Star): 强化学习的引入

$\pi_{0.6}$ 的最强形态是 **$\pi^*_{0.6}$**，它是经过 RL 强化后的版本。
...
- **吞吐量 (Throughput)**: 在折叠衣物、制作咖啡等任务上，$\pi^*_{0.6}$ 的操作速度是 $\pi_{0.5}$ 的 **2倍**。
- **鲁棒性**: 失败率降低了 **50%** 以上。
...
1.  **Phase 1: Pre-training**: 使用海量互联网数据 + 机器人数据，训练出一个 Base Model ($\pi_{0.6}$)。
2.  **Phase 2: Interaction**: 机器人在仿真或真机中大量运行，收集经验。
3.  **Phase 3: Post-training (Recap)**: 使用 Offline RL 算法在收集的数据上进行强化学习，得到 $\pi^*_{0.6}$。

## 4. 总结：Pi 系列进化史

| 模型 | 核心关键词 | 解决的问题 | 类似人类阶段 |
| :--- | :--- | :--- | :--- |
| **$\pi_0$** | Flow Matching | 怎么动？(基础控制) | 婴儿学步 |
| **$\pi_{0.5}$** | Open-World | 怎么适应？(环境泛化) | 幼儿探索世界 |
| **$\pi_{0.6}$** | Action Expert | 怎么做细？(精细操作) | 技工学徒 |
