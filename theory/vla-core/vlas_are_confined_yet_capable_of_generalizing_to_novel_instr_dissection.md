...
> **核心定位**: 通过 Mechanistic Interpretability 揭示 VLA 内部表征的"可组合性"——用 Text Latent Interpolation 将 $\pi_0$ 在 extrapolation 任务上的成功率从 $9\%$ 提升到 $83\%$，同时发现所有 SOTA VLA 普遍存在空间过拟合问题

## ⚡ 快速判斷（30 秒讀完這段就夠了）

| 維度 | 判斷 |
|------|------|
| 核心結論 | VLA 内部学到了可组合的技能表征（text latent），但模型自身无法自主组合它们完成 extrapolation 任务；通过干预 residual stream 可实现 $9\% \to 83\%$ 的跨越 |
...
| 主要風險 | 实验仅基于 LIBERO 仿真环境 + $\pi_0$ 单一模型；real-world 泛化性未知 |

💡 **X-Ray 开场**
这篇论文问了一个根本性问题：VLA 到底是简单地过拟合训练轨迹，还是学到了可组合的内部表征？研究发现，$\pi_0$ 内部确实存储了独立的、可组合的技能表征（text latent），但模型自身无法自主组合它们——就像一个人学会了"把奶酪放进碗里"和"把碗放到柜子上"，却不会"把奶酪放到柜子上"。通过干预 residual stream，可以让模型"突然学会"组合技能，成功率从 $9\%$ 飙升到 $83\%$。对 VLA 研究者的含义是：组合泛化的瓶颈不在表征能力，而在表征的组合机制。
...
| 文本 embedding | 正常编码 | 原始或空白 | 插值 $e^T = (1-\alpha)e_1 + \alpha e_2$ |
| 文本 hidden states | 正常前向传播 | $h^T \gets h^T + \mathcal{T}$ | $h^T \gets h^T + [(1-\alpha)\mathcal{T}_1+\alpha\mathcal{T}_2] - [(1-\alpha)\mathcal{T}_2+\alpha\mathcal{T}_1]$ |
| 是否需要任务描述 | 是 | 否（𝒯 编码了全部任务信息） | 是（但 prompt 可拼接/插值） |
| 适用场景 | in-distribution 任务 | 重建已学任务 | extrapolation 任务 |
| $\pi_0$ 成功率 | 95% (ID) | >80% (无 prompt) | 83% (OOD) |

### 1.2 关键机制 (Key Mechanism)

**Text Latent 识别**：对每个任务，收集所有演示轨迹中每个时间步的文本 token hidden states，跨所有层、所有时间步、所有演示取平均，得到一个固定向量 $\mathcal{T} \in \mathbb{R}^{(L-1)\times|T|\times d}$。

**核心发现**：
- 𝒯 编码了完成任务所需的全部关键信息——即使屏蔽原始 prompt，仅注入 𝒯 到 residual stream，成功率仍 >80%
- $\mathcal{T}$ 可被解码（unembed）为人类不可读的 token 序列，但仍能以 $\sim 70\%$ 成功率驱动模型 → 可用于私有指令或后门攻击
- 两个任务的 $\mathcal{T}_1$ 和 $\mathcal{T}_2$ 可通过线性插值组合，激活两个子行为的顺序执行
...
| L | transformer 层数 | $\pi_0$ 有 $24$ 层 |
| \|T\| | 文本 token 数量 | 取决于 prompt 长度 |
| d | hidden dimension | $\pi_0$ 的 $d$ 值 |
| $\alpha$ | 插值系数 | [0, 1] 线性增长 |
| $\lambda$ | 过渡步数超参 | LIBERO 平均策略步数 |
...
**Step 2 — TLI 干预过程**（假设 $\lambda = 20$）：

| 时间步 i | $\alpha = i/\lambda$ | 干预公式 | 行为效果 |
|----------|---------|----------|----------|
...
**对比**：
- 无 TLI：$\pi_0$ 成功率 9%（无法自主组合）
- 有 TLI：$\pi_0$ 成功率 83%（手动注入组合能力）
- Prompt 切换（$\pi_0^S$）：69%（显式切换 prompt 效果不如表征插值）
...
| 部署约束 | 需要访问模型内部 hidden states | 闭源 API 模型不可用；需本地部署 $\pi_0$ |
| $\lambda$ 超参敏感性 | 中等——大部分任务用默认值即可 | 特殊任务（如酒瓶→碗）需手动调小 $\lambda=14$ |
| 层选择性 | 前 6 层单独干预可达 >20% 成功率 | 可考虑压缩 𝒯 到关键层以减小存储 |

**工程含义**：TLI 是一种 inference-time 干预技术，不改变模型权重、不需要重新训练。核心约束是需要访问模型的 residual stream——这意味着它适用于本地部署的开源模型（如 $\pi_0$），但不适用于闭源 API。对于需要快速验证 extrapolation 能力的场景，TLI 是一个低成本高收益的诊断工具。

## 5. 数据与评测 (Data & Eval)

**基准环境**：LIBERO（仿真），包含三个标准任务套件 + 一个新提出的 OOD 套件。

| 套件 | 任务数 | 描述 | $\pi_0$ baseline |
|------|--------|------|-------------|
...
**评测设置**：
- 每个任务 10 个随机种子 $\times$ 10 个任务 = 100 次运行/套件
...
| $\pi_0$ (baseline) | 9% | 9% |
| $\pi_0 + \text{TLI}$ | **83%** | **83%** |
| $\pi_0 + \text{TLI+}$ ($\text{TEI}+\text{TLI}$) | 83% | 略低 |
| $\pi_0 + \text{TLI*}$ (blank prompt) | 33% | 18% |
| $\pi_0^S$ (prompt switching) | 69% | — |
...
- **组合 extrapolation**：TLI 将 $\pi_0$ 从 $9\%$ 提升到 $83\%$，跨越了 ID→OOD 的鸿沟
...
- **跨模型迁移受限**：$\mathcal{T}$ 是针对 $\pi_0$ 识别的，其他 VLA 架构的 hidden states 形状不同
...
2. **线性插值是表征组合的正确方式**：TLI 使用线性插值 $\alpha \cdot \mathcal{T}_1 + (1-\alpha) \cdot \mathcal{T}_2$，但技能组合可能是非线性的。论文未探索其他组合方式（如门控、attention-based routing）。
...
| $\pi_0$ (Black 2024) | 流匹配 VLA | VLM + flow matching action expert | Open X-Embodiment FT | SOTA ID 性能 |
