...
| **计算开销** | O(⌈T/s⌉·N·C_π) | O(1) 前向 | O(1) 前向 | O(N_probe) 训练 |
| **需要标注** | 需要 token 级语义分区 | 不需要 | 不需要 | 需要概念标注 |
| **OOD 预测力** | r = -0.77 (NMR@10) | 未报告 | 未报告 | 未报告 |

### 1.2 关键机制 (Key Mechanism)

**问题定义**：VLA 策略 π_θ 将多模态观测映射为动作。但 π_θ 可能依赖 spurious correlations（虚假相关）而非因果机制。
...
```
固定各向同性高斯策略 → Fisher 信息矩阵 = σ²I → KL 正比于动作均值差的平方。

4. **NMR 定义**（因果错位度量）：
```
NMR@k = ρ_ISS^(k)(Ω_nuis) = E_X [ |H_ISS^(k)(X) ∩ Ω_nuis| / |H_ISS^(k)(X)| ]
```
Top-k% 因果显著 token 中属于无关区域的比例。理想值 ≈ 0。

> 符号说明：
> - X_t: 多模态上下文（视觉 patch + 指令 token）
> - π_θ: VLA 策略
> - μ_i: 模态条件均值嵌入
> - Ω = Ω_act ∪ Ω_sup ∪ Ω_nuis: 因果空间三分区
...
**输入**：视觉序列 V_t 包含 196 个 patch token（14×14 网格），指令 "pick up the red cup"。

**Step 1 — 干预采样**：
- N = 50 次 Bernoulli 掩码采样，p = 0.5
- 对每次采样 k：V_t,k = V_t ⊙ m_k + V_t^blur ⊙ (1-m_k)
- 计算 â_t,k = π_θ(V_t,k)，δ_k = ||â_t,k - a_t*||²

**Step 2 — 假设数据**：
- 当掩码覆盖了机械臂区域（Ω_act）：δ_k ≈ 0.15（动作变化大 → 因果重要）
- 当掩码覆盖了桌面/杯子区域（Ω_sup）：δ_k ≈ 0.12（动作变化中等 → 因果重要）
- 当掩码覆盖了背景墙壁（Ω_nuis）：δ_k ≈ 0.003（动作几乎不变 → 因果不重要）
...
**Step 4 — 计算 NMR**：
- Top-10% 显著 token（196×0.1 = 19 个 token）
- 若其中 2 个来自 Ω_nuis（背景墙壁）：NMR@10 = 2/19 ≈ 0.105
- 若其中 8 个来自 Ω_nuis：NMR@10 = 8/19 ≈ 0.421

**Step 5 — 预测泛化**：
- 根据论文 Figure 3，NMR@10 = 0.105 → 预期成功率 ≈ 75%
- NMR@10 = 0.421 → 预期成功率 ≈ 30%
...
| **内存占用** | 空间复杂度 O(T·H·W)，存储 T 帧的显著性图。对于 10 帧序列 + 224×224 图像 ≈ 50KB，可忽略 |
| **时间步_stride** | 论文用 stride s 降低计算频率（每隔 s 步计算一次 ISS），再通过线性插值恢复连续流。s=5 可减少 5× 计算量 |
| **部署约束** | 需要预定义 token 级语义分区（Ω_act, Ω_sup, Ω_nuis）。这依赖额外的分割模型或人工标注，是工程落地的主要瓶颈 |
| **与训练解耦** | ISS/NMR 完全在推理阶段计算，不修改模型权重，可作为任何 VLA 的后置诊断工具 |
| **模糊核选择** | V_t^blur = V_t * K_σ 作为 baseline。σ 的选择影响干预强度——太大则分布偏移，太小则干预效果不显著 |
...
1. **Token 空间可语义分割**：论文假设 VLA 的视觉 token 空间中存在清晰的语义边界（Ω_act、Ω_sup、Ω_nuis 可分离）。但这依赖预定义的分割标注，而实际 VLA 的 token 可能是 entangled 的——一个 token 可能同时编码物体和背景信息。

2. **高斯策略假设**：Action MSE 作为 KL 散度代理依赖于「固定各向同性高斯策略」。如果 VLA 使用混合高斯、离散动作空间或确定性策略，这个等价关系不成立。

3. **静态基线有效性**：用训练集均值 μ_i 作为干预 baseline 假设训练分布能覆盖模态语义子空间。但对于罕见物体或新场景，均值可能不是一个合理的 baseline。

4. **因果分区不变性**：Ω_act、Ω_sup、Ω_nuis 的定义是任务无关的（机械臂永远是 Ω_act）。但在某些任务中（如「擦拭背景墙壁」），背景可能变成任务相关区域。
