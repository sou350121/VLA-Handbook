...
- LL： goal-conditioned 策略，输入 (o_source, o_target)，输出动作块 a_c = ⟨a_0, ..., a_{n-1}⟩

**Stage 2 — Rollout 收集**：当前策略 π_t 部署，每个上下文 (s_0, l) 执行 K 次 rollout，保留第一个成功轨迹。
- 上下文类型 1：环境重置上下文 (o_0 ~ ρ_reset, g ~ G_l)
- 上下文类型 2：回放上下文 (o_0 = o_N^* from D_t, g ~ G_l) — 用已收集轨迹的终态作为新起点，探索标准重置无法到达的状态

**Stage 3 — 数据集聚合**：
- ORCHID：D_{t+1} = D_t ∪ ℛ_t，从头训练（防遗忘，但计算量递增）
- ORCHID-ft：D_{t+1} = ℛ_t，从 π_t 微调（计算量恒定，但有遗忘风险）
...
| 符号 | 含义 |
|------|------|
| ζ^j | 扩散步 j 的 noisy 子目标序列 |
| ζ^0 | 目标子目标序列（从 expert/成功轨迹提取） |
| α_j, β_j | 噪声调度系数 |
| v_φ | 速度预测网络（U-Net） |
| o_0*, g | 初始观测 + 文本目标 |
...
> 符号与本文保持一致：π_HL 为高层扩散规划器，π_LL 为底层控制器，ζ 为视觉子目标序列，ℛ_t 为第 t 轮成功轨迹集，D_t 为第 t 轮训练数据集。
...
**iter 1 rollout 收集**：
- 对每个上下文，π_0 执行 K=5 次 rollout
- 假设 100 个上下文中 35 个至少有一次成功 → ℛ_1 包含 35 条成功轨迹
- 关键：ℛ_1 中的子目标是 LL_0 实际到达的状态观测——这些是"LL 真正能到达"的子目标

**iter 1 蒸馏**：
- HL_1 在 ℛ_1 上训练：不再拟合人类演示的子目标，而是拟合 LL_0 实际到达的子目标 → HL 开始生成"LL 可到达"的子目标
- LL_1 在 ℛ_1 上训练：学到的是"在 HL_0 生成的子目标模式下应该做什么动作" → LL 开始适配 HL 的计划风格
...
| 环境交互成本 | 每轮 K × |C(D_t)| 次 rollout | 同左 | 这是最大瓶颈——CALVIN 仿真中每次 rollout 约数秒 |
...
| CALVIN D→D | 34 | 150 demo/任务 | 人类遥操作 | MTLC（单任务）+ LH-MTLC（长程连续任务） |
...
**数据来源**：论文 Table 1，3 seeds 均值 ± 标准误。
...
- **依赖二值奖励**：仅用 R∈{0,1}，无法利用密集奖励信号（如果有的话）
...
2. **仿真到实机的迁移性**：CALVIN 的 D→D split 虽然是 in-distribution，但仍是仿真环境。实机上的 HL-LL 耦合问题可能更复杂（传感器噪声、动力学不确定性）。
...
- **建議章節路徑**：先讀 §4（方法）→ 再看 §5 实验（Table 1 + reachability error）→ 可跳 §3.1（GC-POMDP 形式化，标准设定）
