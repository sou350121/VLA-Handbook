...
**Step 1: 人体重建**
- Exo 视频 → 手部 keypoint：指尖位置 x_h = (0.32, 0.11, 0.51)m
- 相似变换 T_{h→r}（人类到机器人标定）：缩放 0.95，平移 (0.01, -0.01, 0.02)m
- 目标机器人指尖位置：T_{h→r}(x_h) = (0.314, 0.095, 0.507)m

**Step 2: 手部 IK 求解**
- 初始种子：q_hand⁰ = 零位（手指伸直）
- 残差：e_hand = p_i^r(q_hand⁰) - (0.314, 0.095, 0.507) = (0.15, 0.0, 0.6) - 目标 = (-0.164, -0.095, 0.093)m
- Levenberg-Marquardt 迭代 5 次后：
  - q_hand* = [0.3, -0.1, 0.2, 0.15, ...] (20 个手指关节角度)
  - 残差降至：\|e_hand\| ≈ 0.003m（3mm）

**Step 3: 手臂 IK 求解**
- 手腕目标从手部帧提取：p_wrist* = (0.25, 0.05, 0.45)m
- 阻尼 Jacobian IK 求解 7-DoF 手臂：
  - q_arm* = [0.2, -0.3, 0.1, 0.4, -0.15, 0.25, 0.1]
  - 手腕位置误差：\|W(q_arm*) - p_wrist*\| ≈ 0.005m

**Step 4: FK 验证**
- 正向运动学验证：F_U(q_combined) = (W, R, P)
- 指尖最终位置：P(q*) = (0.312, 0.097, 0.505)m
- 与目标偏差：\|P(q*) - T_{h→r}(x_h)\| ≈ 0.004m（4mm）

**Step 5: Guard 规则**
- 检查：E_b(q̃) = 0.000016 < E_b(q_base) - ε = 0.0001 - 0.00001
- 接受：q_new = q̃（改进超过阈值 ε）

**训练时 DS-HKC 梯度流**（同一轨迹）：
- 预测 q̂ 有 5mm 指尖误差 → L_tip = (0.005)² = 0.000025
...
| URDF 绑定 | PrimeU 专属 | 迁移到新平台需重新标定 T_{h→r} + 重新设计 staged IK 顺序 |
...
2. **人类-机器人相似变换 T_{h→r} 可精确标定** — 论文假设存在一个全局的相似变换将人类指尖坐标映射到机器人空间。但对于非刚性差异（如手指比例不同），单一相似变换可能不够。
