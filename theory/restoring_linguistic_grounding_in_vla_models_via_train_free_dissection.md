...
| 主要風險 | 仅验证了 3 个 VLA 架构 + LIBERO 仿真 + Franka 实机，泛化到更大规模 VLA（如 π0 系列生产版）待验证 |

💡 **X-Ray 开场**
这篇论文发现了一个令人不安的现象：当前主流 VLA 模型（π0、π0.5、OpenVLA-OFT）在收到与场景矛盾的指令时，仍然会执行视觉上合理的动作——比如场景中没有白碗，指令说"拿起白碗"，机器人照样去拿黑碗。语言指令几乎被忽略了。作者将这种现象命名为**「语言失明」（Linguistic Blindness）**，并提出了一种免训练的注意力重校准方法 IGAR，在推理时重新平衡视觉与语言 token 的注意力分布，使模型重新"听从"指令。对 VLA 研究者而言，这意味着我们之前评估的"高成功率"可能大部分来自视觉启发式，而非真正的语言理解。
...
| **适用架构** | — | transformer-based VLA（π0/π0.5/OpenVLA-OFT 已验证） |
| **超参调优** | — | 统一超参跨任务/跨架构（τ=20, γ=3.0, ρ=0.4, p=0.6） |
...
- **直觉**：如果模型真正理解语言，矛盾指令 ℓ̃ 应该导致任务失败（SR 下降），LGS 接近正常 SR；如果模型忽略语言，SR 不变，LGS ≈ 0
- **完美 grounding 模型**：LGS = SR(f_θ, ℓ)（矛盾指令下完全失败）
- **完全失明模型**：LGS ≈ 0（矛盾指令下照样成功）
...
- H ∈ R^{N×D}：中间 transformer 层的隐藏状态
- φ(d) > γ=3.0 的维度入选 D_spike（top-k=5）
...
- 先将文本汇 token 的注意力缩减到 60%，释放预算 Ω
- 将 Ω 按比例分配给非汇语言 token
- 直觉：不是全局增强语言，而是精准地从"过度聚集"的地方释放注意力，补给"被忽视"的指令 token

> 符号与本文保持一致：ℓ=正常指令, ℓ̃=矛盾指令, f_θ=VLA 策略, SR=任务成功率, H=隐藏状态, A=注意力权重, S=汇 token 集合, V=视觉 token 集合, T=文本 token 集合
...
→ 模型看到黑碗，直接去拿。"white"这个矛盾信号被完全忽略。SR ≈ 95%（照样成功）。

**IGAR 干预后**：

Step 1: 检测 — 黑碗 token 的 spike ratio φ(d) = 4.2 > γ=3.0，max|H|=28 > τ=20 → 判定为视觉汇。
...
→ "white" token 的注意力从 0.02 升到 0.05，模型开始"注意到"指令与场景的不一致。SR 下降到 40-60%，LGS 从 ~5 提升到 ~40。

## 4. 工程视角 (Engineering View)

| 工程维度 | 数值 | 含义 |
|----------|------|------|
| 推理延迟增量 | +7.0%（98.7→105.6 ms） | 在 RTX 5090 上测得，对 10Hz 控制频率影响可控 |
| 计算开销 | 仅前 16 层干预 | 不增加参数量，纯前向计算 |
| 内存占用 | 零额外存储 | 不需要缓存额外状态或训练数据 |
| 部署成本 | 即插即用 | 修改推理代码即可，无需重新训练或微调 |
| 超参稳定性 | 统一超参跨任务 | τ=20, γ=3.0, ρ=0.4, p=0.6 对所有 30 个 LIBERO 任务通用 |
| 架构兼容性 | transformer-based VLA | 对 π0（流匹配）需离散对比公式（τ_detect=0.15, τ_max=0.50） |
...
| 跨架构通用 | π0/π0.5/OpenVLA-OFT | 基于 transformer 通用注意力机制 |
...
2. **统一超参跨任务有效**：τ=20, γ=3.0, ρ=0.4, p=0.6 对所有 30 个任务通用——这可能掩盖了任务间的差异
...
- [π0 架构](https://www.physicalintelligence.company/download)
