...
> **核心定位**: 用 VLM 作为离线语义导师，把 7B/4B 级 VLA 教师蒸馏成 158M 轻量策略，推理延迟降 3.28× 而成功率几乎不损失

## ⚡ 快速判斷（30 秒讀完這段就夠了）

| 維度 | 判斷 |
|------|------|
| 核心結論 | VLM 生成的阶段锚点+多帧方向信号可作为离线语义监督，将 OpenVLA-7B 蒸馏为 158M 学生策略，平均仅差 0.27%，推理速度提升 3.28× |
...
| 组件 | 输入 | 输出 | 训练阶段 | 推理阶段 |
|------|------|------|----------|----------|
| 教师 VLA (OpenVLA-7B / π₀.₅-4B) | 图像 + 指令 | 7-DoF 动作 (x,y,z,θ,ϕ,ψ,gripper) | 生成专家轨迹 | ❌ 不使用 |
...
- **完整路径** (L_full): 图像 x_t + 指令 τ + VLM 描述 d_t → 预测动作
- **图像路径** (L_img): 图像 x_t + 指令 τ（屏蔽描述）→ 预测动作
- 总损失: L_total = L_full + α · L_img
...
**目标**: 让学生同时学会"跟着动作数值走"和"理解语义上下文"，α 控制两条路径的权重平衡。
...
| π_S^{(k,j)} | 学生策略对第 k 步第 j 维的预测分布 |
| w_j | 动作维度权重（旋转通道 ×2 补偿量级） |
| α | 双路径平衡超参（搜索 {0.3, 0.5, 0.8, 1.0}） |
| K | 动作 chunk 长度 = 5 |

> 符号与本文保持一致：L_full 用完整输入（图像+指令+描述），L_img 屏蔽描述通道。α=1.0 表示两条路径平等对待，α=0.5 表示更依赖视觉控制。
...
| 指标 | OpenVLA-7B | π₀.₅-4B | VLA-AD 学生 (158M) |
|------|-----------|---------|--------------------|
| 参数量 | 7B | 4B | 158M (可训练 8.6M) |
| 推理速度 (RTX 4090) | 3.8 Hz | ~5.3 Hz (推算) | 12.5 Hz (OpenVLA 蒸馏) / 13.2 Hz (π₀.₅ 蒸馏) |
| 速度提升 | 基线 | 基线 | 3.28× / ~2.5× |
...
- **22 GPU-hours** 的训练成本相比全量微调 OpenVLA-7B 降低了 10-20×，使得快速迭代成为可能
...
**Trade-off**:
- 学生策略的上限受教师数据质量约束——如果教师在某类任务上表现差，学生无法超越（但在 π₀.₅ 教师上，学生在两个套件上超过了教师，说明语义引导有时能突破教师的行为分布）
- 9 阶段分类器是规则启发式的，迁移到新机器人平台时需要重新设计规则

## 5. 数据与评测 (Data & Eval)

**数据集**: LIBERO 三个 in-domain 套件
- libero_object: 10 任务 × 20 episode = 200 trials
- libero_spatial: 10 任务 × 20 episode = 200 trials
- libero_goal: 10 任务 × 20 episode = 200 trials
...
| VLA-AD 学生 (α=1.0/0.5/1.0) | 匹配教师 (差 0.27%) | 匹配教师 | 匹配教师 | 94.73% |
| π₀.₅-4B 教师 | ~89% | ~86% | ~90% | ~88% |
| VLA-AD 学生 (同 α) | 96.5% (+7.22) | 93.0% (+6.90) | 94.0% (+0.50) | 94.5% |

**超参选择**:
- α 搜索 {0.3, 0.5, 0.8, 1.0}
- libero_object / libero_goal: α=1.0（视觉主导，VLM 仅作语义锚点）
- libero_spatial: α=0.5（空间关系需要更多显式语言描述）
...
| 能力 | 场景 | 原因 |
|------|------|------|
| 大幅压缩模型 | 7B→158M (44×) | Long-CLIP + LoRA rank=8 架构高效 |
| 保持成功率 | LIBERO 三套件接近教师 | 语义引导减少 compounding error |
| 超越次优教师 | 超过 π₀.₅-4B 在 2/3 套件 | VLM 语义捕获了可迁移的操作结构 |
...
| 仅验证了 2 个教师 | OpenVLA-7B 和 π₀.₅-4B，未测试其他架构（如 RT-1, Octo） |
