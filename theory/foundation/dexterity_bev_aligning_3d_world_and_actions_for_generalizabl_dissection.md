...
| 可以跳過 | 如果你只关心纯2D VLA（如RT-1/$\pi_0$）且不涉及跨平台迁移 |
| 落地可行性 | 中（需要相机标定+深度图，但提供了无深度时的Vertex Spectrum方案） |
| 主要風險 | 实验仅在仿真+4套真实硬件上验证，未见大规模跨场景测试；BEV构建依赖高质量深度 |

💡 **X-Ray 开场**

现有VLA模型（如$\pi_0$、X-VLA）从2D VLM继承而来，用RGB图像做输入。但机器人操作本质上是3D问题——同一物体在不同相机角度下像素完全不同，导致策略学到的是"相机特定的捷径"而非真正的3D操作能力。Dexterity-BEV的核心发现是：把所有相机观测统一投影到一个共享的鸟瞰图（BEV）坐标系后，策略对相机位姿变化变得鲁棒，且同一模型权重可以跨不同机器人本体部署。对VLA研究者意味着，2D VLM + 3D对齐表示 这条路线比纯3D表示（点云/体素）更可行——因为你仍然可以利用web-scale预训练的2D视觉 backbone。
...
| 组件 | 2D VLA基线（$\pi_0$/X-VLA） | Dex-BEV |
|------|------------------------|---------|
| **视觉输入** | RGB图像 $H \times W \times 3$ | RGB + Vertex Map（对齐到BEV的3D坐标）+ 可选BEV Image |
...
3. **Vertex Spectrum（无深度时的替代）** — 当某些相机没有深度传感器时，对每个像素采样$M$个离散深度假设 $\{d_j\}$，每个假设反投影后变换到BEV帧，形成一个 $M \times 3$ 的 volumetric coordinate grid，经轻量编码器编码为2D位置嵌入加到RGB特征上。
...
对每个像素采样$M$个深度假设（线性递增离散化LID），每个假设反投影并变换到BEV帧，形成$M \times 3$的体素坐标网格，经轻量编码器编码为2D位置嵌入。
...
| 维度 | 分析 |
|------|------|
| **BEV图像构建开销** | 需要将所有相机点云聚合后做正交投影。对于$H \times W$的BEV分辨率，复杂度 $O(N_{\text{cam}} \times H_{\text{img}} \times W_{\text{img}})$，但可GPU并行。推理时每个forward pass都需要重建BEV。 |
...
| 指标 | $\pi_0$ | X-VLA | 2D Ablation | Dex-BEV |
|------|-----|-------|-------------|---------|
...
- 在跨本体场景（RoboTwin用LIBERO训练的同一权重），Dex-BEV显著优于$\pi_0$（59.0% vs 31.4%）
...
| 未见与更多SOTA对比 | 仅对比$\pi_0$和X-VLA，未与RT-2/OpenVLA等对比 |
...
- 对比基线: $\pi_0$ [[6](https://www.pi0.ai/)], X-VLA [[55](https://arxiv.org/abs/2411.19630)]
