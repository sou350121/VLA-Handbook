...
> **核心定位**: 用卡尔曼滤波（KF）做运动学校正，替代推测解码（SD）中昂贵的 re-inference，同时用运动学变异度动态调节接受阈值，实现 VLA 推理 $1.48\times\sim1.57\times$ 加速且几乎不损失成功率。

## ⚡ 快速判斷（30 秒讀完這段就夠了）

| 維度 | 判斷 |
|------|------|
| 核心結論 | 将传统机器人运动学（KF）引入 VLA 推测解码，用运动学校正替代 re-inference，用运动学变异度动态调节接受阈值，在 LIBERO 上实现 $1.48\times\sim1.57\times$ 加速，成功率几乎不变 |
...
| 接受阈值 | 无（严格 greedy） | 固定 r=9/15/20 | 动态 $r\in[5,15]$，由 $K_{\text{var}}$ 调节 |
| 硬件部署 | GPU | GPU | CPU (KF/阈值) + GPU (draft/verify) |
| LIBERO-Goal SR | 77.0% | 75.4% (r=9) | 75.6% |
| LIBERO-Goal Speed | $1.00\times$ | $1.19\times$ | $1.54\times$ |
| LIBERO-Long SR | 54.4% | 49.2% (r=9) | 48.8% |
| LIBERO-Long Speed | $1.00\times$ | $1.12\times$ | $1.48\times$ |

### 1.2 关键机制 (Key Mechanism)

**问题 1：SD 出错后的 re-inference 太贵**

VLA 每个 action slice 有 $7$ 个 DoF（$X, Y, Z, \theta_X, \theta_Y, \theta_Z, \text{gripper}$），需要 $7$ 步自回归推理。SD 用 draft model 预生成多个 token，但一旦某个 token 出错，naive SD 需要 re-inference 整个 slice——这对 $7\text{B}$ 验证模型来说是 $3.92\ \text{TFLOPs}$ 的开销。
...
当 $K_{\text{var}}$ 变化剧烈（$\Delta K_{\text{var}}$ 大）时，$\Delta r$ 变小 $\to$ 阈值收紧 $\to$ 更保守地接受 token。
...
r^{t+1} = r^t + Δr^t
```
- $\Delta K_{\text{var}}^t = K_{\text{var}}^t - K_{\text{var}}^{t-1}$：运动学变异度的变化量
- $K_{\text{var}}^S$：预采样得到的参考变异度
- r_max = 15, r_min = 5：阈值上下界
- $\phi$：形状参数（查表获得）
- 直觉：$\Delta K_{\text{var}}$ 越大 $\to$ exp 项越小 $\to$ $\Delta r$ 越小 $\to$ 阈值收紧 $\to$ 更保守
...
**Step 4 — KF 补偿**：
- SD 接受 DoF 0~5 的动作
- KF 基于 $\text{Cache}_{X\sim G}^{AC=10}$ 预测 DoF 6
- 最终 slice = concat(SD[0:6], KF[6:7])

**Step 5 — 阈值调整**：
假设 $\Delta K_{\text{var}}^t = 0.05$, $K_{\text{var}}^S = 0.1$, $\varphi = 2$：
```
...
| 场景 | 表现 | 原因 |
|------|------|------|
| 短期动作序列（LIBERO-Goal/Spatial） | $1.54\times\sim1.57\times$ 加速，SR 几乎不变 | KF 短期预测精度高，SD 错误率低 |
| 长程任务（LIBERO-Long） | $1.48\times$ 加速，SR 从 $54.4\%$ 降至 $48.8\%$（$-5.6\%$） | 长程任务中误差累积，KF 补偿质量下降 |
| 不同任务环境自适应 | 动态阈值自动调节 $r\in[5,15]$ | Kvar 驱动的阈值调整对不同环境鲁棒 |
...
| 新机器人平台 | 需要重新预采样建表 | $\tau$, $\varphi$, $r_{\max}$, $r_{\min}$ 参数依赖预采样，迁移成本高 |
...
2. **Token-ID 差异与运动学变异度存在可映射关系**：论文通过 $\text{Map}(\cdot)$ 函数将 token 差异映射到 $K_{\text{var}}$，但这个映射函数的精度直接影响阈值调整的质量。论文未报告 $\text{Map}$ 函数的拟合误差。

3. **预采样建表可行**：$\tau$, $\varphi$ 等参数需要"在多种环境中预采样"获得。这意味着每个新任务/机器人组合都需要一次预采样过程——对于快速迭代开发场景，这个成本不可忽视。
