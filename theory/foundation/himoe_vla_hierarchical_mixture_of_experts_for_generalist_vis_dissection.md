...
目标：学习一个向量场 v_θ，将高斯噪声逐步变换为目标动作分布。
...
直觉：这是一个监督对比损失，作用于路由概率分布。同一动作空间的 token 的路由向量被拉近，不同动作空间的被推远。排除 anchor 自身避免了 exp(1/β) 的自相似主导项。
...
- **共享专家**贡献约 0.057 的 CALVIN 增益（4.012 → 3.955 的差距，Table 10），说明 heterogeneity-agnostic 的通用计算不可忽略
...
| 新物体/干扰泛化 | xArm7 67.6% vs. π₀ 55.9%（Table 3） | 仅桌面操作场景 |
| 双臂协调 | ALOHA Fold-Shorts 等任务 | 仅 ALOHA 平台 |
| 兼容不同训练配方 | 替换 FLOWER 的密集专家 → 4.35→4.49 | 仅在 CALVIN 上验证 |
...
4. **分层深度固定**：AS-MoE → HB-MoE → Dense 的三层结构是人工设计的，未探索自适应分层或数据驱动的分层优化。
...
| π₀ | Flow-matching VLA | 密集 Transformer + Flow | OXE 混合 | 通用操作 |
...
- **建议章节路径**：先读 §3（方法，尤其是 HiMoE 架构和 AS-Reg/HB-Reg）→ 再看 §4.3（消融实验，理解每层/每个损失的贡献）→ 可跳 §2（相关工作，除非需要文献综述）
