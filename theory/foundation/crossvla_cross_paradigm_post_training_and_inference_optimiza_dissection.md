...
> **核心定位**: 首次打通离散自回归（OpenVLA）与连续流匹配（$\pi_{0.5}$）两大 VLA 架构的后训练（DPO）壁垒，并提供 DoRA 在 VLA 上的首个系统性评估 + 流匹配推理延迟解剖。
...
| 主要風險 | Workshop draft（14 页），$\pi_{0.5}$ + DPO 在 Goal/Long-horizon 上未完成（dev pod 断连） |

💡 **X-Ray 开场**
VLA 世界分裂成两派：OpenVLA 用离散 token 自回归预测动作，$\pi_{0.5}$ 用连续流匹配生成动作块。两派的后训练方法互不相通——DPO 在离散模型上很简单，但在流匹配模型上需要概率流 ODE 积分，计算代价极高。CrossVLA 用一个基于 MSE 的 surrogate log-prob 绕过了这个问题，让 DPO 可以跨范式工作。对研究者而言，这意味着你可以用同一套后训练协议处理任何架构的 VLA。
...
| 组件 | OpenVLA (离散 AR) | $\pi_{0.5}$ (连续流匹配) | CrossVLA 统一接口 |
|------|-------------------|-------------------|-------------------|
| 动作表示 | $7 \times 256$-bin 离散 token | 7-DoF 连续 action chunk | VLABase Protocol 抽象 |
...
| x_0 | 高斯噪声 |
| x_t | 插值: $(1-t) \cdot x_0 + t \cdot x_1$ |
| v_target | 目标速度: x_1 - x_0 |
| $v_\theta(x_t, t, \text{obs})$ | 模型预测的速度场 |
| T_eval | 评估时间步数 = 4 |
...
**与扩散模型的联系**: 扩散模型的理论下界将 MSE 与 $\log$-likelihood 通过已知因子连接（$\Vert x_1 - x_0 \Vert^2 \cdot \sigma^2(t)$）。论文将该因子吸收到 DPO 温度参数 $\beta$ 中，直接使用原始 MSE。
...
$\beta=0.1$, $\text{lr}=5e\text{-}5$, $\text{batch\_size}=1$, $\text{max\_steps}=500$, $\text{warmup}=100$。

> 符号与本文保持一致：$c^+$ 为 chosen chunk（SFT 成功采样的动作），$c^-$ 为 rejected chunk（添加动作噪声 $\sigma=0.1\to0.4$ 后采样的动作）。
...
模型预测速度场 $v_\theta(x_{0.5}, 0.5, \text{obs}) = [0.38, -0.47]$
...
正向的 $\Delta$ 意味着模型正确地将 chosen 排在 rejected 之前，loss 推动参数进一步增大这个差距。

## 4. 工程视角 (Engineering View)

### 训练开销

| 指标 | OpenVLA + DoRA | $\pi_{0.5}$ + LoRA |
|------|----------------|-------------|
| 可训练参数 | 34.08M (0.45%) | 类似量级 |
| 峰值 GPU 显存 | 26.17 GB (DoRA) / 17.93 GB (LoRA) | 约 20 GB |
| DPO 训练步数 | 500 steps | 500 steps |
| 每步 batch | 1 | 1 |
| 单 suite 训练时间 | ~30 min (1$\times$H20) | ~30 min |

### DoRA 额外开销

DoRA 相比 LoRA 的额外成本：每层需要 materialize 一个 out$\times$in 的完整权重张量（计算 $W_{\text{eff}} = m \odot \dots$）。对于 OpenVLA 的 128 个 LoRA-target Linear 层（hidden=4096）：
...
| Denoise loop $\times 10$ (action expert) | ~220 ms | **78.6%** |
| **总计** | **~280 ms** | **100%** |
...
缓存命中率 82.1%（cosine similarity $\geq 0.95$），但：
...
| 数据集 | 用途 | 规模 |
|--------|------|------|
| LIBERO (4-suite) | 主要评测 | 130 unique tasks, 10 tasks/suite $\times$ 5 trials/task |
| modified_libero_rlds | Multi-view pretrain 数据 | 50 episodes/suite $\times$ 30 anchor times $\times$ 4 suites = 6000 samples |

### DPO 数据生成

- 每 suite ~200 (chosen, rejected) chunk pairs
- chosen: SFT 成功 rollout 采样的 action chunks
- rejected: 添加动作噪声 $\sigma=0.1\to0.4$（ramping）后采样的 chunks

### 评测协议

- LIBERO 4 suites: Spatial, Object, Goal, Long-horizon
- 每 suite: 10 tasks $\times$ 5 trials/task = 50 trials
...
| Suite | $\pi_{0.5}$ SFT (本文) | $\pi_{0.5}$ 论文 | 差异 |
|-------|-----------------|-----------|------|
...
$\pi_{0.5}$ 复现与 openpi 论文在 $\pm 2$ pp 内一致，确认评测管线正确。
...
| 能力 | 证据 | 限制 |
|------|------|------|
| 跨范式 DPO | $\pi_{0.5}$ + LoRA + DPO 在 Spatial/Object 上稳定训练 | Goal/Long-horizon 未完成（dev pod 断连） |
| DoRA 稳定性 | Object suite 零种子方差（3 seeds × 38/50 = 76%） | 仅在 OpenVLA 上验证，$\pi_{0.5}$ 上未测试 DoRA |
...
| 场景 | 失败表现 | 原因 |
|------|----------|------|
| KV-Cache chunk-level | 成功率 $100\% \to 80\%$，速度 $+43\%$ 更慢 | 缓存复用导致 rollout drift + 检查开销 |
| KV-Cache token-level (prefix) | 成功率降至 0% | 过时的 prefix K/V 破坏 suffix attention |
| $\pi_{0.5} +$ DPO on Goal/Long-horizon | 未完成 | dev pod tunnel 断连（基础设施问题，非方法问题） |
...
2. **DPO 温度 $\beta$ 吸收 prefactor 的合理性**: 论文将流匹配 prefactor $\Vert x_1 - x_0 \Vert^2 \cdot \sigma^2(t)$ 吸收到 $\beta$ 中。这意味着 $\beta$ 不再纯粹是 DPO 温度，而是与数据分布耦合。不同数据集可能需要不同的 $\beta$。
...
| 方法 | 关注点 | 架构 | 训练方式 | 适用场景 |
|------|--------|------|----------|----------|
| **CrossVLA (本文)** | 跨范式后训练 | OpenVLA $+ \pi_{0.5}$ | Offline DPO + surrogate logp | 离散/连续 VLA 后训练 |
...
- [$\pi_{0.5}$](https://arxiv.org/abs/2501.18689)
