...
3. **多尺度对称亲和**：使用 SymSoftmax 在多个距离尺度 R ∈ R 上计算样本间亲和度，交叉侧质量平衡确保吸引力和排斥力总量相等（反对称性）。
...
| 符号 | 含义 |
|------|------|
| G | 生成的假设矩阵 B×G×S（B=批大小, G=假设数, S=动作维度） |
| G̅ | stop-gradient 版本的 G |
| Y | 参考池 [G̅, N⁻, P⁺]（自身+负样本+专家正样本） |
| s_norm | 平均 pairwise 距离，用于归一化 |
| R | 多尺度集合（控制亲和度计算的粒度） |
| V | 聚合漂移场（吸引-排斥的净效果） |
| sg(·) | stop-gradient 操作，防止目标被梯度更新 |

> 符号与论文 §IV 保持一致。S = H·d_a（chunk 模式）或 S = d_a（step-wise 模式）。
...
**Step 2 — 亲和度**（设 R=1, s_norm=4.0, 简化 SymSoftmax ≈ softmax）：
...
**反向传播更新 θ**：网络参数被推动，使得下次生成时 x̂₁ 更接近漂移目标方向——即朝向专家、远离其他假设。经过足够多训练步后，网络学会了"直接输出接近 [3,4] 的动作"，推理时不再需要漂移场计算。
...
| 对比 DP 加速比 | ~100× | 论文 §I（100 NFE → 1 NFE） |
...
- **部署确定性**：部署时 σ 被禁用，策略是确定性的 1-NFE 前向传播，行为可复现，便于安全验证。
...
1. **漂移场可辨识性**：论文假设 V_{p,q}(x) ≡ 0 ⇒ q = p（零漂移意味着分布匹配），这是一个充分条件假设，未严格证明在所有生成器架构下成立。
...
- **建议章节路径**：先读 §III（Drifting Models 预备知识）→ 再看 §IV-B（DBP 漂移目标推导）→ 然后 §IV-C（DBPO 随机适配器）→ 可跳 §II（相关工作综述）
