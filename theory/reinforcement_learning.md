...
│   (真实/仿真/数据集) 环境 Env                                                │
│   ┌──────────────────────────────────────────────────────────────────────┐   │
│   │  state s_t / obs o_t                      reward r_t                 │   │
│   │  (含视觉/触觉/本体感知/语言上下文)         (稀疏/密集/偏好/成功信号)     │   │
│   └───────────────┬───────────────────────────────────────────────┬──────┘   │
│                   │                                               │          │
│                   │ o_t                                           │ r_t      │
│                   v                                               v          │
│        ┌───────────────────────┐                     ┌──────────────────────┐ │
│        │  策略 π(a|s) / πθ      │   (选动作)          │  价值 V/Q / 优势 A     │ │
│        │  Policy / Actor        │───────────────────>│  Critic               │ │
│        └───────────┬───────────┘                     └───────────┬──────────┘ │
│                    │ a_t                                         │ (学习信号) │
│                    v                                             │ TD / GAE   │
│             ┌───────────────┐                                    │           │
│             │  动作 a_t      │<───────────────────────────────────┘           │
│             │  (关节/末端/扭矩/离散token/chunk)                                │
│             └───────┬───────┘                                                │
│                     │ step                                                   │
│                     v                                                        │
│            环境转移：s_{t+1} ~ P(·|s_t, a_t)                                  │
│                                                                              │
│   数据流（经验）                                                             │
│   ┌──────────────────────────────────────────────────────────────────────┐   │
│   │  轨迹/回放缓存 D = {(o_t, a_t, r_t, o_{t+1})}                          │   │
│   │  - on-policy: 只用当前策略采样的数据（PPO/A2C…）                       │   │
│   │  - off-policy: 可复用旧数据（SAC/TD3/DQN…）                             │   │
│   │  - offline RL: 只给定数据集 D（CQL/IQL/Recap…）                          │   │
│   └──────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│   算法家族（你可以把它当“目录索引”）                                          │
│   - Value-based:  学 Q*(s,a) 再贪心选动作（DQN…）                             │
│   - Policy-based: 直接学 πθ（REINFORCE…）                                     │
│   - Actor-Critic: πθ + V/Q（PPO/SAC/TD3…）                                    │
│   - Model-based:  学模型 p(s'|s,a) 做想象/规划（World Model / MPC…）          │
│                                                                              │
│   VLA/机器人常见落点（工程视角）                                              │
│   - BC/IL → RL: 先模仿获得可用策略，再 RL 做性能上限与长序列优化              │
│   - Sim2Real: 仿真并行采样 + 域随机化/系统辨识 + 真机少量校准                 │
│   - 分层控制: 高层(慢)规划/技能，中层(中)策略，底层(快)RT/接触闭环(S0)         │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
