...
| 可以跳过 | 如果你只关心纯反应式 VLA（如 π0-FAST、OpenVLA）且不做推理增强；或你的场景不需要 OOD 泛化 |
...
| 维度 | 传统 VLA（π0-FAST 等） | 早期 CoT-VLA | DeepThinkVLA |
|------|----------------------|-------------|-------------|
...
| 训练方式 | 行为克隆 / SFT | SFT on CoT-annotated data | 两阶段：SFT 冷启动 → RL 因果对齐 |
| CoT 角色 | 无 | 装饰性（风格模仿，不参与决策） | 功能性（RL 后成为规划信号） |
| OOD 泛化 | 差（31.6pp drop） | 差（32.0pp drop，与无推理持平） | 较好（24.4pp drop，CoT 真正起作用） |
| 推理延迟 | 基线 | 4× 基线（AR 顺序生成动作） | 0.175× 基线（Mask CoT 时，并行解码动作） |
| 参数规模 | ~2.9B（π0-FAST） | ~2.9B | 2.9B（基于 π0-FAST 重构） |
...
> 符号与本文保持一致：π_θ 为当前策略，π_ref 为 SFT 参考策略，ε 为 PPO clip 参数（通常 0.2），β 为 KL 惩罚系数。
...
- 动作生成（双向注意力，并行）：A = [平移(x+5,y+0,z-2), 旋转(0,0,π/4), 夹爪(闭合)] × h=10 chunks
...
- 收集 G=4 条轨迹的 rollout：
  ...
  - τ_2: 成功 → R(τ_2) = 1.0
  - τ_3: 失败（抓空）→ R(τ_3) = 0.0
  - τ_4: 失败（放错位置）→ R(τ_4) = 0.0
- 组内优势计算：
  - mean = 0.5, std = 0.5
  - Â(τ_1) = Â(τ_2) = (1.0 - 0.5) / 0.5 = +1.0
  - Â(τ_3) = Â(τ_4) = (0.0 - 0.5) / 0.5 = -1.0
- 更新：τ_1 和 τ_2 中的所有 token（包括 CoT 和动作）被强化；τ_3 和 τ_4 中的 token 被抑制
...
- RL 模型 + Mask CoT：成功率降到 57.8%（drop 27.7pp）→ 证明 CoT 在 RL 后真正参与了决策
...
| 维度 | 数值/约束 | 工程含义 |
|------|----------|---------|
| 模型参数量 | 2.9B | 单卡 80GB 可推理，SFT 需 ≥8×80GB GPU |
| SFT 训练 | batch=8, grad_accum=2 | 有效 batch size = 16，需多卡分布式 |
| RL 训练 | GRPO + online rollout | 需要模拟器环境交互，计算密集 |
| 推理延迟（完整 CoT） | 约 4× π0-FAST（AR CoT 生成） | CoT 增加延迟，但可 Mask |
| 推理延迟（Mask CoT） | 0.175× π0-FAST | 跳过 CoT 生成，直接并行解码动作，比基线还快 |
| 动作 chunk size | h（论文未明确，通常 10-15） | 影响控制频率和推理粒度 |
| 控制维度 | d=7（6-DoF + 夹爪） | 标准机械臂配置 |
| 输入相机数 | num_images_in_input=2 | 支持多视角，非单目 |

**关键工程洞察**：混合注意力解码器的并行动作生成不仅解决了模态冲突，还带来了意想不到的好处——当 Mask CoT 时（跳过推理阶段），模型可以直接并行解码动作，比 π0-FAST 的 AR 解码快 5.7 倍。这意味着在实际部署中，可以在简单任务上跳过 CoT（低延迟），在复杂任务上启用 CoT（高可靠性）。
...
| 真实机器人数据 | AGILEX ALOHA 遥操作 | 3 个任务 × 若干轨迹 | 真实世界验证 |

**CoT 标注策略**：
- Stage 1：夹爪状态变化检测 → 关键帧 → 云端 LVLM 标注 → 人工抽检
- Stage 2：本地 VLM 微调 → 自动标注中间帧 → Schema 检查 + 时间一致性
...
| 训练成本高 | SFT 需 8×80GB GPU，RL 需多节点 | 高（工程门槛） |
...
4. **π0-FAST 权重是合适的起点**：所有实验基于 π0-FAST，未测试从零训练或其他 VLA 骨干（虽然有 Qwen3-VL 的泛化性实验，但那是另一个方向）。
...
| 方法 | 关注点 | 架构 | 训练方式 | 适用场景 |
|------|--------|------|---------|---------|
| π0-FAST | 快速反应式 VLA | AR 解码器 | 行为克隆 | 分布内桌面操作 |
