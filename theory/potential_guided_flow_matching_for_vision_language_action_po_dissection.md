...
x = [a; s] ∈ R^{H·(dₐ+1)}
```
- a ∈ R^{H·dₐ}：动作块（H=30 步，dₐ=23 维）
- s ∈ [0,1]^{H}：成功势函数向量，每步对应一个成功概率估计
...
> 符号说明：
> - θ：网络参数
> - σ ∈ [0,1]：Flow 插值时间（非物理时间）
> - v_θ：学习的速度场，分解为 v_θ⁽ᵃ⁾（动作分量）和 v_θ⁽ˢ⁾（势函数分量）
> - u：目标速度 = x₁ - x₀
> - sg(·)：stop-gradient 操作，防止基线估计回传梯度

**直觉**：动作部分像 AWR——好动作被强调，差动作被抑制；势函数部分像监督学习——成功和失败都平等对待，确保模型不会高估失败。

## 3. 带数字走一遍：玩具例子 (Worked Example)

假设一个简化场景：H=3 步的动作块，dₐ=2 维动作。

**样本**：轨迹 i 在时间 t 的 chunk
- 阶段标签：yᵢ,ₜ = 1（该阶段成功完成）
- 目标端点：x₁ = [a₁; s₁] = [0.5, 0.3, 0.8, 0.2, 0.6, 0.9, 1.0, 1.0, 1.0]
  - 前 6 维是动作 (3步×2维)，后 3 维是势函数 (broadcast y=1)
...
**Step 4: Loss 计算**（假设 σ=0.5 时的回归误差）
```
动作部分: ||v_θ⁽ᵃ⁾ - u⁽ᵃ⁾||² = 0.15
...
| 推理延迟 (K=5) | 5×Flow 积分（scoring 免费） | 1×Flow + 5×Critic forward | ForesightFlow 更快（无额外 Critic 前向） |
...
| Autonomous 数据 | 100 π₀.₅ 自采集 | 100 π₀.₅ 自采集 |
...
2. **势函数平均作为 chunk 级评分有效**：Q̂(c,a) = mean(sₖ) 假设所有时间步的重要性相同。对于某些任务，关键步骤（如抓取）可能比辅助步骤（如接近）更重要。
3. **One-Step 估计器的排名保真度**：Kendall τ = 0.80-0.86 意味着约 15-20% 的排名不一致。在安全关键场景中，这个误差率可能不可接受。
4. **仿真到真实的迁移**：仿真使用 π₀.₅  backbone，真实使用 π₀ backbone——两者架构不同，比较不是完全公平的。
...
- π₀ (Flow Model for Robot Control): https://arxiv.org/abs/2410.04164
