...
> 符号与本文保持一致：$\hat{O}/\hat{B}/\hat{A}$ 表示预测值，$O/B/a$ 表示真实值，$g(\cdot)$ 表示冻结 ViT 编码。
...
- chunk_1 (t=$-6$ s): 前方车辆开始减速，加速度 $a_x = -2\ \text{m/s}^2$
- chunk_2 (t=$-3$ s): 前方车辆急刹，加速度 $a_x = -5\ \text{m/s}^2$
...
**LDM 预测输出**（在 t=0 步）：
- 动作预测 $\hat{A}_0$: 自车加速度 $-3.5\ \text{m/s}^2$（开始刹车），转向 $0$（保持车道）
- Camera token $\hat{O}_0^{\text{front}}$: 预测前方车辆距离缩短的视觉变化
...
**Vision Renderer 渲染**：
将 $\hat{O}_0^v$ (7 个视角的 camera token) 渲染为高保真图像，反馈给 LDM 作为下一轮观测。直接解码 camera token 会得到模糊图像（因为 token 是"可能未来的均值"），Renderer 恢复高频细节（刹车灯亮起、距离拉近）。
...
| Stride | 课程学习 $1$ s → $3$ s | 扩展 horizon 而不增加序列长度 |
| 注意力复杂度 | 块稀疏 → $O(N)$ 而非 $O(N^2)$ | 长序列训练可行性的关键 |
