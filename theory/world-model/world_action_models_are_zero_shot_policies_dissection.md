...
**输入**：
- 视觉上下文 $o_{0:l}$: 3 帧 RGB 历史 (720p, VAE 压缩到 $64\times64$ latent)
- 语言 c: "push the cup to the edge"
- 本体感知 $q_l$: 末端位置 $[x=0.3,\ y=0.2,\ z=0.5]$, 关节角度 $[\theta_1\ldots\theta_7]$
...
**关键数值**：
- 单次推理延迟：150ms (优化后)
- 控制频率：7Hz ($1000\,\text{ms} / 150\,\text{ms} \approx 6.7$，chunk 重叠实现平滑)
...
| 优化类别 | 技术 | 加速比 | 性能影响 |
|----------|------|--------|----------|
| 算法级 | DreamZero-Flash (解耦调度) | $4\times$ | 无 |
| 系统级 | KV Cache + 异步推理 | $6\times$ | 无 |
| 系统级 | 算子融合 + 并行 | 3× | 无 |
...
| 约束 | 要求 | DreamZero 方案 |
|------|------|----------------|
| 控制频率 | $\geq 5\,\text{Hz}$ 才能平滑操作 | 7Hz (达标) |
| 显存 | 14B 模型 + KV cache | 80GB A100/H100 (单卡) |
| 观测延迟 | 相机$\to$推理$\to$动作 $<200\,\text{ms}$ | 150ms 推理 + 相机帧率 30Hz |
...
- DreamZero 选择：$640\times 480$ VAE latent，平衡点
...
| 跨具身学习 | 从 AgiBot G1 $\to$ YAM | 视频是具身无关的表示 |
| 长序列操作 | 多步任务 (打开$\to$取出$\to$放置) | 自回归视频预测提供长期规划 |
