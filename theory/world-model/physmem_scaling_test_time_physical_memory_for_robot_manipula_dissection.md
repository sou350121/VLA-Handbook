...
|------|------|------|-----------|-------------|
| **VLM Planner** (πθ^H) | 观测 o_t, 任务 τ, 活跃原则 P_t | 高层选项 ω_t（离散决策） | 每决策步 | 推理（θ 固定） |
| **Low-level Executor** (π^L) | 选项 ω_t, 当前观测 | 电机指令 | 高频控制循环 | 推理（固定） |
| **Episodic Memory** | (o, ω, r, c, s) 经验元组 | 原始经验存储 | 每步写入 | 推理时积累 |
...
**三层记忆架构**：
- **情景记忆（Episodic）**：存储原始经验 e = (o, ω, r, c, s)，容量上限 N_max，按符号状态过滤
- **工作记忆（Working）**：存放待验证假设，每个假设带有置信度分数（来自支持/反驳证据）
- **长期记忆（Long-term）**：存放已验证原则，带重要性衰减（γ=0.995），随时间遗忘过时知识

**科学记忆循环四阶段**：

1. **经验收集 + 共振检查**：计算共振分数 ρ(e, P_active) = |{p ∈ P_active : consistent(e, p)}| / |P_active|。ρ=1 时经验强化现有原则（静默）；ρ<1 时触发巩固（学习新东西）
2. **假设生成**：按符号相似度聚类经验，对规模 ≥ n_min 的簇用反思模型 f_φ 生成 typed 假设（Avoid/Prefer/Sequence）
3. **行动级归因**：仅用特定行动类型的结果更新假设置信度，隔离规划决策与执行噪声
4. **验证与提升**：conf(h) ≥ τ_p（通常 0.8）且支持证据 ≥ 3 → 提升为长期原则；conf(h) ≤ τ_r 且反驳证据 ≥ 2 → 驳回假设
...
**目标**：在不更新 VLM 参数 θ 的前提下，通过学习原则集 P* 使得：
```
...
> 符号与本文保持一致：o=观测, ω=选项, r=奖励(0/1), c=上下文, s=符号状态, P=原则集, H=假设集, E=经验集, ρ=共振分数, conf=置信度, τ_p/τ_r=提升/驳回阈值, α=学习率, γ=遗忘率。

## 3. 带数字走一遍：玩具例子 (Worked Example)

假设 Ball Navigation 任务，系统前 5 个 episode 的交互过程：

**Episode 1-2（无原则，ρ ≈ 0.2）**：
- 动作：「高速推球穿过拱门」→ 结果：球撞到障碍物（r=0）
- 经验 e_1 = (obs_1, push_high_speed, 0, "ball_nav", {action_type: "push", speed: "high"})
- 共振检查：P_active = ∅（无原则），ρ 未定义 → 触发巩固
- 聚类：e_1 单独成簇（|C|=1 < n_min=3，不生成假设）

**Episode 3-4（积累中）**：
- e_2：「高速推球」→ 球滚出界（r=0），同样符号状态
- e_3：「中速推球」→ 球到达目标（r=1），符号状态 {action_type: "push", speed: "mid"}
- 现在 |C_high|=2, |C_mid|=1，仍不满足 n_min

**Episode 5（触发假设生成）**：
- e_4：「高速推球」→ 再次撞障碍（r=0）
- 现在 C_high = {e_1, e_2, e_4}，|C_high|=3 ≥ n_min=3
- f_φ 生成假设 h_1: [Avoid] "不要在拱门前使用高速推球"

**Episode 6-8（验证阶段）**：
- 系统遇到类似场景时，h_1 处于工作记忆中
- e_5：「高速推球（拱门前）」→ r=0（支持 h_1）
- e_6：「中速推球（拱门前）」→ r=1（与 h_1 不冲突，h_1 只说高速不好）
- conf(h_1) 更新：α · (2/3) = 0.1 · 0.67 = 0.067（从 0 起步）
- 继续积累...

**Episode 10（提升为长期原则）**：
- conf(h_1) 经过多次验证后达到 0.82 ≥ τ_p=0.8
- |E_support| = 5 ≥ 3
- → h_1 提升为长期原则 P
- e_1, e_2, e_4, e_5 被「折叠」（folded）进原则，从情景记忆中清除
- 后续推理时，h_1 作为 top-k 原则注入 VLM prompt

**结果**：共振分数从 ρ≈0.2 提升到 ρ≈0.7+，系统不再在拱门前使用高速推球。
...
| 零参数更新 | θ 固定 | 无需 GPU 训练，纯 CPU 即可运行记忆循环 |
...
**真实世界平台**（论文 §IV-A）：
- 硬件：xArm6 机械臂 + fin-ray 软体夹爪 + Intel RealSense D435（俯视 1280×720 / 腕部 640×480）
- 三个任务，每个 10-20 个 episode，每轮运行 30+ 分钟
- Parts Organization：6 个不规则零件放入 3×10 网格，最小化占用格子
...
| Parts Organization 提升 | -1 → 9.7（有记忆 vs 无记忆 ≈ 0） | 论文 Figure 8 |
| Ball Navigation 提升 | 14.7 vs 0.7（有记忆 vs 无记忆） | 论文 Figure 8 |
| 共振分数演进 | 0.2 → 0.9（10 episode 内） | 论文 Figure 7 |
| 新颖球体迁移成功率 | 10% → 40%（有测试时适应 vs 无） | 论文 §V-C |
...
| LLM 假设生成可靠 | f_φ 用 VLM/LLM 从经验簇生成假设 | 假设质量受 LLM 能力限制；可能生成错误/冗余假设 |
| 行动可归因 | 行动级归因假设单一行动类型与结果有因果关系 | 多行动耦合时归因可能混淆 |
| 任务可分解为离散选项 | 使用 options framework（ω = ⟨I, π, β⟩） | 不适用于需要连续力/阻抗控制的精细操作 |
| 原则可自然语言表达 | 所有原则都是人类可读文本 | 某些物理直觉可能难以用语言精确描述 |
| 环境静态或缓变 | 原则重要性用 γ=0.995 衰减 | 快速变化环境中衰减可能不够快或太快 |
