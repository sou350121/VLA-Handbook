...
## 一张图看懂：RECAP 把 RL 蒸馏成“偏好调节监督”

```text
                          目标：让 VLA 在真实部署中持续变强
                 (higher success / higher throughput / lower failure)
...
要点：
- 训练形式像 SFT，但“标签”来自交互 + value/adv（这是 RL 的精华）。
- 失败/恢复轨迹不是噪声，而是最值钱的数据来源。
```
