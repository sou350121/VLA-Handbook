...
VLA Handbook 的每日内容由 [照见 Pulsar](https://github.com/sou350121/Pulsar-KenVersion) 系统自动驱动，不是人工维护的静态文档。

- **全自动运行** — 33 个 cron job，每天 09:15 北京时间开始，无需人工触发
- **自愈 Watchdog** — 15 项健康检查，pipeline 故障（RSS 中断 · LLM 超时 · 评分缺失）自动恢复，不会静默丢失数据
- **信号评分前置** — 每天 30+ 篇论文先经过 ⚡/🔧/📖/❌ 评级，只有精选进入 LLM 深度分析，节省 80%+ 推理成本
- **月度假设校准** — 系统维护 19 条 VLA 领域假设，每月根据实际触发率自动更新置信度，判断力持续迭代
- **双周预测验证** — 每两周报告含可验证预测，下一期必须打分（✅ 已验证 / ❌ 落空 / ⏳ 待观察），正确率透明可查
