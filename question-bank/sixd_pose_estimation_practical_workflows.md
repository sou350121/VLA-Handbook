# 6D 姿态估计：工程实战工作流（RGB / RGB-D / 多视角 / 跟踪）

> 目标：把 6D pose 相关概念与落地流程写成一份**可直接复用的工程笔记**（pipeline、关键细节、常见坑、与 VLA 的接口）。
...
### 0.15 常见工程术语（做 Eye-in-Hand / Tracking 时经常会被问）

- **ROI（Region of Interest）**
  - **一句话**：把瓶盖那一小块区域裁出来，只在 ROI 内跑 pose/refine/tracking，速度更快也更不容易被背景干扰。

- **bbox vs mask**
  - **bbox**：一个矩形框（粗定位）。
  - **mask（分割掩码）**：像素级“这个像素是不是瓶盖”。
  - **一句话**：瓶盖这种小目标/反光/背景乱时，mask 往往比 bbox 稳（因为能把背景扣干净）。

- **Point Cloud（点云）**
  - **一句话**：由深度图反投影得到的一堆 3D 点。
  - **提醒**：你现在的瓶盖场景是 **只有 RGB**，所以这个通常用不上；但面试会问“有 RGB-D 你会怎么做”，你就能接上 ICP/点云 refine。

- **Re-init（重新初始化）**
  - **一句话**：tracking 置信度持续低/丢跟时，不硬跟，回到“检测/分割 → 初始 pose”重新抓回目标。

- **Residual / Inliers（残差 / 内点）**
  - **residual（残差）**：当前 pose 有多不对（误差越小越好）。
  - **inliers（内点）**：RANSAC 认为“符合这个 pose”的匹配点有多少（越多越可信）。
  - **一句话**：残差/内点数就是 tracking 的“置信度来源”，决定要不要 re-init。

- **motion blur（运动模糊）**
  - **一句话**：相机/手动太快 + 曝光时间长，画面糊了，关键点/边缘会不稳。

- **rolling shutter（滚动快门畸变）**
  - **一句话**：图像是一行一行曝光的；相机快速运动时，物体会“变形”，几何估计更容易抖。

- **Eye-in-Hand（眼在手上）**
  - **一句话**：相机装在末端上，相机会跟着末端动。
  - **你要能说清楚的 3 个 $T$**：
    - $T_C^E$：相机在末端下（手眼标定得到，固定）。
    - $T_E^B(t)$：末端在基座下（机器人每帧给）。
    - $T_C^B(t)=T_E^B(t)\cdot T_C^E$：当前相机在基座下（算出来，用来把 $T_{obj}^C$ 换到 $T_{obj}^B$）。
